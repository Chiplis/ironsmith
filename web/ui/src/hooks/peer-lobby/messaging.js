...
export function usePeerLobbyMessaging(base, servicesRef) {
  ...
  const requestResync = useCallback((reason = "Resyncing with host...", { forceCheckpoint = true } = {}) => {
    ...
    safeSend(conn, {
      ...
      forceCheckpoint,
      matchId: relayMatchId(matchStartPayloadRef.current),
      prefixHash: actionHistoryRef.current.at(-1)?.prefixHash || EMPTY_ACTION_PREFIX,
    });
    setStatus(reason, true);
    return true;
  }, [setStatus, updateMultiplayer]);

  // A failed state recovery pauses play. Retry a bounded number of times on our own
  // so a transient failure (a stale payload, a host mid-action) heals without the
  // player having to reconnect; a persistent failure surfaces after the last attempt.
  const scheduleResyncRetry = useCallback(() => {
    const retry = resyncRetryRef.current;
    if (retry.timer) return true;
    if (retry.attempts >= 3) return false;
    retry.attempts += 1;
    retry.timer = window.setTimeout(() => {
      retry.timer = null;
      const session = multiplayerRef.current;
      if (session.role !== "client" || !session.matchStarted || !awaitingStateResyncRef.current) return;
      if (resyncInProgressRef.current) return;
      requestResync(`Retrying state recovery (${retry.attempts}/3)...`);
    }, 4000 * retry.attempts);
    return true;
  }, [requestResync]);
  useEffect(() => () => {
    if (resyncRetryRef.current.timer) window.clearTimeout(resyncRetryRef.current.timer);
  }, []);
  ...
  useEffect(() => {
    const recoverForegroundSession = () => {
      ...
      // Relay rooms always re-check on foreground; a direct-peer guest does so only
      // while play is paused on a failed recovery, when it is the way back in.
      if (!isRelayId(session.lobbyId) && !(session.role === "client" && awaitingStateResyncRef.current)) return;
  ...
  const applyStateResync = useCallback(
    async (message) => {
      ...
      if (message.suffix === true) {
        const baseSequence = Number(message.baseSequence);
        if (!isTrustedMultiplayerSecurityMode(sessionSecurityMode(currentSession))
            ...
            || baseSequence !== Number(currentSession.lastAppliedSequence)
            || !matchingActionPrefix(actionHistoryRef.current, baseSequence, message.basePrefix)) {
          throw new Error("Recovery suffix does not extend the current accepted match");
        }
        const entries = message.actions;
        if (!Array.isArray(entries) || Number(message.lastSequence) !== baseSequence + entries.length) throw new Error("Recovery suffix is incomplete");
        let prefix = message.basePrefix;
        for (let i = 0; i < entries.length; i++) {
          const entry = entries[i];
          prefix = actionPrefixHash(prefix, entry);
          if (Number(entry.seq) !== baseSequence + i + 1 || entry.prefixHash !== prefix) throw new Error("Recovery suffix prefix mismatch");
        }
        const savepoint = await servicesRef.current.createSequencedActionValidationSnapshot();
        let nextState;
        try {
          nextState = await replayTrustedActions(currentGame, entries, baseSequence);
        } catch (error) {
          await servicesRef.current.restoreSequencedActionValidationSnapshot(savepoint);
          throw error;
        } finally { await savepoint.release?.(); }
        for (const entry of entries) await servicesRef.current.appendAppliedSequencedAction(entry);
        ...
        resyncRetryRef.current.attempts = 0;
        safeSend(hostConnectionRef.current, { type: "resync_ack", protocolVersion: PROTOCOL_VERSION, lastSequence: Number(message.lastSequence) });
        setStatus(`Resynced with trusted host at action ${message.lastSequence}`);
        await servicesRef.current.drainPendingSequencedActions();
        return;
      }
      ...
      // A resync whose transcript is shorter than ours is normally a stale message
      // and is acknowledged without replaying. The exception is the current trusted
      // host explicitly rolling back actions it did not accept; Verified never rolls back.
      ...
      if (isTrustedMultiplayerSecurityMode(securityMode)) {
        // Trusted mode: the host's accepted transcript is authoritative. The local
        // transcript is not required to be a prefix of it (a guest that diverged
        // must be able to recover), so the payload is validated on its own terms
        // and adopted wholesale. Every check runs before any local state changes,
        // and the engine replay is wrapped in a savepoint, so a rejected or failed
        // resync leaves the seat exactly where it was and a retry stays possible.
        ...
        const acceptedActions = withActionPrefixes(actionEntries.map((entry) => ({
        ...
        acceptedActions.forEach((entry, index) => {
          if (Number(entry.seq) !== index + 1) {
            throw new Error(`Resync transcript is not contiguous at action ${index + 1}`);
          }
        });
        const finalSequence = Number(acceptedActions.at(-1)?.seq || 0);
        const messageLastSequence = Number(message?.lastSequence ?? finalSequence);
        if (messageLastSequence !== finalSequence) {
        ...
        const expectedHostSeat = normalizePlayerIndex(matchPayload.currentHostPlayerIndex)
          ...
          ?? 0;
        const replaySavepoint = await servicesRef.current.createSequencedActionValidationSnapshot();
        let nextState;
        try {
        ...
        }, acceptedActions, localEntry.index ?? currentSession.localPlayerIndex ?? 0);
        ...
        nextState = typeof currentGame.uiState === "function"
          ...
          try {
            await servicesRef.current.restoreSequencedActionValidationSnapshot(replaySavepoint);
          } catch {
            // Keep the replay error as the actionable failure.
          }
          throw error;
        } finally {
          await replaySavepoint.release?.();
        }
        stateRef.current = nextState;
        setState(nextState);
        ...
        actionHistoryRef.current = acceptedActions;
      ...
      resyncRetryRef.current.attempts = 0;
        ...
        const retrying = scheduleResyncRetry();
        setStatus(
          retrying
            ? "State recovery failed; retrying shortly."
            : "State recovery failed. Play is paused; return to this tab or reconnect to retry.",
          true
        );
        throw error;
      } finally {
    ...
    async (message) => {
      ...
      switch (message.type) {
        ...
        case "player_presence":
          ...
          return;
        case "trusted_recovery_needed":
          if (message.matchId === relayMatchId(matchStartPayloadRef.current)) requestResync("Recovering host acceptance", { forceCheckpoint: false });
          return;
        ...
        case "apply_action": {
          ...
          if (isTrustedMultiplayerSecurityMode(sessionSecurityMode(multiplayerRef.current))) {
            servicesRef.current.acknowledgeTrustedAction(hostConnectionRef.current, message, result);
          }
          return;
        }
        default:
  ...
  const configurePeerConnection = useCallback(
    (conn) => {
      ...
      conn.on("data", (message, transport) => {
  ...
  const sendDirectPeerMessage = useCallback(
    (peerId, payload) => {
      ...
      if (existingRoute) {
        return safeSend(existingRoute, payload);
      }
      if (session.role === "host") {
        let conn = null;
  ...
  const handleClientMessage = useCallback(
    async (conn, message) => {
      ...
      case "crypto_material_response":
        resolveCryptoMaterial(message);
        return;
      case "trusted_action_ack":
        servicesRef.current.receiveTrustedAck(conn, message);
        return;
      case "trusted_command":
        try { await servicesRef.current.acceptTrustedCommand(conn, message); }
        catch (error) {
      ...
      case "ziffle_reveal_token_request":
        ...
        case "join_request": {
          const session = multiplayerRef.current;
          if (isTrustedMultiplayerSecurityMode(sessionSecurityMode(session))
              && (message.trustedProtocol !== 2 || message.runtimeVersion !== RUNTIME_VERSION)) {
            safeSend(conn, { type: "reject", protocolVersion: PROTOCOL_VERSION,
              reason: "Trusted multiplayer requires the same application and engine version on every seat" });
            conn.close(); return;
          }
        ...
        case "resync_request": {
          ...
          if (!needsFullStateResync({
            forceCheckpoint: message.forceCheckpoint === true || message.force === true
              || (isTrustedMultiplayerSecurityMode(sessionSecurityMode(session))
                && (message.matchId !== relayMatchId(matchStartPayloadRef.current)
                  || !matchingActionPrefix(actionHistoryRef.current, requesterSequence, message.prefixHash))),
          ...
          try {
            await sendHostedStateMessage(conn, {
              ...
              lastSequence: nextSession.lastAppliedSequence,
              requesterSequence,
              requestPrefixHash: message.prefixHash,
              requestMatchId: message.matchId,
              forceCheckpoint: message.forceCheckpoint === true || message.force === true
              || (isTrustedMultiplayerSecurityMode(sessionSecurityMode(session))
                && (message.matchId !== relayMatchId(matchStartPayloadRef.current)
                  || !matchingActionPrefix(actionHistoryRef.current, requesterSequence, message.prefixHash))),
