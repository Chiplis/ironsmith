// SHA-256 for public checkpoint hashing on HTTP LAN origins, where SubtleCrypto
// is unavailable. Signing and encryption still require WebCrypto.
...
export function sha256Bytes(value) {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let i = 0; i < 8; i++) hash[i] += next[i];
  }
  const result = new Uint8Array(32);
  const output = new DataView(result.buffer);
  for (let i = 0; i < 8; i++) output.setUint32(i * 4, hash[i]);
  return result;
}
