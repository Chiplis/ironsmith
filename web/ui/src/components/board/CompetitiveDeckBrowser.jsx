...
export default function CompetitiveDeckBrowser({ players, targetIndex, onTargetChange, onSelect }) {
  ...
  const results = useMemo(
    () => searchCatalogEntries(catalog?.decks, query),
    [catalog, query],
  );
  ...
  return (
    <section className="grid gap-2 border border-[rgba(154,126,82,0.42)] bg-[rgba(8,9,9,0.7)] p-3" aria-label="Competitive deck catalog">
      ...
      <input className={fieldClass} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Boros, Energy, Guide of Souls..." aria-label="Buscar en catálogo" />
      ...
      <div className="grid max-h-[220px] gap-2 overflow-y-auto pr-1">
        {results.map((entry) => (
          <div key={entry.id} className="flex items-center justify-between gap-3 border border-white/10 px-2 py-2">
            <div className="min-w-0">
              ...
              <div className="truncate text-[10px] text-[#8b806b]">{(entry.colors || []).join(" ")} {(entry.mechanics || []).join(" · ")}</div>
            </div>
            <Button type="button" variant="ghost" size="sm" className="h-8 shrink-0 border border-[#9a7e52]/55 px-3 text-[11px] font-bold uppercase tracking-wide text-[#d8bf7a]" disabled={Boolean(busyId)} onClick={() => handleSelect(entry)}>
