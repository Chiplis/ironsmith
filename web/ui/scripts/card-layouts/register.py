...
def main():
    ...
    # Reviewed region/style annotations are printing data, not card-name branches
    # in the renderer. Preserve them when regenerating the OCR catalog.
    reviewed = json.loads((Path(__file__).resolve().parents[2]/'tests/card-frame-regression-cases.json').read_text())
    for case in reviewed:
        if case.get('registration'):
            registrations = [r for r in registrations if r['id'] != case['id']]
            registrations.append(case['registration'])
