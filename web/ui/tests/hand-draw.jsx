...
export function HandDrawFixture() {
  const [count, setCount] = useState(7);
  const hand = Array.from({length: count}, (_, i) => ({ ...cards[0], id: i + 1, stable_id: i + 1 }));
  const player = {id: 0, can_view_hand: true, hand_cards: hand};
  return <I18nProvider><GameContext.Provider value={{...context, state: {...context.state, players: [player], snapshot_id: count, zone_transitions: count > 7 ? [{id: `draw-${count}`, from_zone: "library", to_zone: "hand", owner: 0, new_object_id: count}] : []}}}><HoverProvider><DragProvider><CombatArrowProvider><TooltipProvider>
    <main style={{padding:40}}>
      <button onClick={() => setCount(n => n + 1)}>Draw card</button>
      <div style={{height:350, marginTop:100, display:"flex", justifyContent:"center"}}><HandZone layout="mobile-fan" player={player} onInspect={()=>{}} isExpanded /></div>
    </main>
  </TooltipProvider></CombatArrowProvider></DragProvider></HoverProvider></GameContext.Provider></I18nProvider>;
}
createRoot(document.getElementById("root")).render(<HandDrawFixture />);
