    ...
    try {
      ...
      for (const [label, delays] of [
        ...
        if (!delays.fail) {
          ...
          } else if (stage.dataset.frameMode === 'custom') {
            // The mask failed, so the card's own frame carries the art crop --
            // and it is the card's details, with no panel stacked on top.
            check(stage.querySelector('.interactive-card-frame__art img'), 'Missing art in the custom frame');
