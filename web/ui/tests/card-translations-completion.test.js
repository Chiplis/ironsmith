...
for(const [type,failed,expected,calls]of [['',false,'Instantáneo',1],['',true,'Instant',1],['Instantáneo',false,'Instantáneo',0]])test(`complete official type fields ${type||'missing'} ${failed}`,async()=>{
 ...
 globalThis.__translationTest={cardRouteKey:()=> 'counterspell',fetchScryfallLocalizedCardTranslation:async()=>{requests++;if(failed)throw Error('Offline');return {typeLine:'Instantáneo'};},loadGeneratedTextTranslation:async()=>null,hasTranslatedFields:()=>true,translationForFace:value=>value,baseAssetUrl:()=>"http://localhost/"};
 try{
  const code=source.replace(/^import .*;\n/gm,'');
  const mod=await import(`data:text/javascript,${encodeURIComponent('const {cardRouteKey,fetchScryfallLocalizedCardTranslation,loadGeneratedTextTranslation,hasTranslatedFields,translationForFace,baseAssetUrl}=globalThis.__translationTest;\n'+code+'\n//'+Math.random())}`);
