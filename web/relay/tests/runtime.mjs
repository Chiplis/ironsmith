...
export async function startRelay(allowed = 'http://localhost:5173', bindings = {}) {
  ...
  const mf = new Miniflare({ modules: true, script: bundle.outputFiles[0].text, compatibilityDate: '2026-08-01',
    bindings: { ALLOWED_ORIGINS: allowed, ...bindings }, durableObjects: {
      ROOMS: { className: 'LobbyRoom', useSQLite: true }, DIRECTORY: { className: 'LobbyDirectory', useSQLite: true },
      TOURNAMENTS: { className: 'TournamentRegistry', useSQLite: true }, DISPUTES: { className: 'WitnessDisputes', useSQLite: true } } });
