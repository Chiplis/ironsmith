/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const wasmgame_registerExternalCardSourcesJson: (a: number, b: number, c: number) => [number, number, number, number];
export const wasmgame_registerExternalCardSources: (a: number, b: any) => [number, number, number];
export const wasm_start: () => void;
export const wasmgame_selectGrandMeleeStack: (a: number, b: number, c: number) => [number, number, number];
export const wasmgame_new: () => number;
export const wasmgame_setAutoChooseSingleObjectDecisions: (a: number, b: number) => void;
export const wasmgame_setLimitedRangeOfInfluence: (a: number, b: any) => [number, number];
export const wasmgame_setAttackDirection: (a: number, b: number, c: number) => [number, number];
export const wasmgame_setTeams: (a: number, b: any) => [number, number];
export const wasmgame_setDeployCreatures: (a: number, b: number) => void;
export const wasmgame_setSharedTeamTurns: (a: number, b: number) => [number, number];
export const wasmgame_setSharedTeamMemberOrder: (a: number, b: number, c: any) => [number, number];
export const wasmgame_reset: (a: number, b: any, c: number) => [number, number];
export const wasmgame_resetEmpty: (a: number, b: any, c: number) => [number, number];
export const wasmgame_finishPuzzleSetup: (a: number) => [number, number];
export const wasmgame_startMatch: (a: number, b: any) => [number, number, number];
export const wasmgame_revealHiddenObject: (a: number, b: any) => [number, number, number];
export const wasmgame_revealHiddenSlot: (a: number, b: any) => [number, number, number];
export const wasmgame_revealHiddenPosition: (a: number, b: any) => [number, number, number];
export const wasmgame_revealHiddenPositions: (a: number, b: any) => [number, number, number];
export const wasmgame_exportHiddenCardOpening: (a: number, b: bigint) => [number, number, number];
export const wasmgame_previewCryptoRequirements: (a: number, b: any) => [number, number, number];
export const wasmgame_injectTranscriptRandomSeeds: (a: number, b: any) => [number, number];
export const wasmgame_applyVerifiedHiddenLibraryShuffle: (a: number, b: any) => [number, number, number];
export const wasmgame_validateMatchConfig: (a: number, b: any) => [number, number, number];
export const wasmgame_snapshot: (a: number) => [number, number, number];
export const wasmgame_uiState: (a: number) => [number, number, number];
export const wasmgame_lastSnapshotPerf: (a: number) => [number, number, number];
export const wasmgame_lastDispatchPerf: (a: number) => [number, number, number];
export const wasmgame_lastWorkCounters: (a: number) => [number, number, number];
export const wasmgame_lastReplayExecutionPerf: (a: number) => [number, number, number];
export const wasmgame_lastAdvanceUntilDecisionPerf: (a: number) => [number, number, number];
export const wasmgame_registrySize: (a: number) => number;
export const wasmgame_preloadRegistryStatus: (a: number) => [number, number, number];
export const wasmgame_preloadRegistryChunk: (a: number, b: number) => [number, number, number];
export const wasmgame_objectDetails: (a: number, b: bigint) => [number, number, number];
export const wasmgame_snapshotJson: (a: number) => [number, number, number, number];
export const wasmgame_autocompleteCardNames: (a: number, b: number, c: number, d: number) => [number, number, number];
export const wasmgame_isKnownCardName: (a: number, b: number, c: number) => number;
export const wasmgame_setLife: (a: number, b: number, c: number) => [number, number];
export const wasmgame_addLifeDelta: (a: number, b: number, c: number) => [number, number];
export const wasmgame_forfeitPlayer: (a: number, b: number) => [number, number, number];
export const wasmgame_forceNextDieRoll: (a: number, b: number) => void;
export const wasmgame_setDaytime: (a: number, b: number) => [number, number, number];
export const wasmgame_isDaytime: (a: number) => number;
export const wasmgame_hasDayNight: (a: number) => number;
export const wasmgame_drawCard: (a: number, b: number) => [number, number, number];
export const wasmgame_moveHandCardToBattlefieldFaceDown: (a: number, b: number, c: bigint, d: number) => [bigint, number, number];
export const wasmgame_forceTurnFaceUp: (a: number, b: number, c: bigint) => [number, number];
export const wasmgame_addCardToHand: (a: number, b: number, c: number, d: number) => [bigint, number, number];
export const wasmgame_addCardToZone: (a: number, b: number, c: number, d: number, e: number, f: number, g: number) => [bigint, number, number];
export const wasmgame_addCardsToZones: (a: number, b: any) => [number, number, number];
export const wasmgame_setCombatDamageAssignment: (a: number, b: bigint, c: bigint, d: number) => void;
export const wasmgame_combatDamageAssignmentPlayer: (a: number, b: bigint) => number;
export const wasmgame_setCombatDamageAssignmentForPlayer: (a: number, b: number, c: bigint, d: bigint, e: number) => [number, number];
export const wasmgame_setAttackingBand: (a: number, b: any) => [number, number];
export const wasmgame_drawOpeningHands: (a: number, b: number) => [number, number];
export const wasmgame_loadDemoDecks: (a: number) => [number, number];
export const wasmgame_loadDecks: (a: number, b: any) => [number, number, number];
export const wasmgame_cardLoadDiagnostics: (a: number, b: number, c: number, d: number, e: number) => [number, number, number];
export const wasmgame_sampleLoadedDeckSeed: (a: number, b: number) => [number, number, number];
export const wasmgame_previewCustomCard: (a: number, b: any) => [number, number, number];
export const wasmgame_createCustomCard: (a: number, b: any) => [bigint, number, number];
export const wasmgame_advancePhase: (a: number) => [number, number];
export const wasmgame_enterAdditionalCombatPhase: (a: number) => [number, number];
export const wasmgame_setAutoCleanupDiscard: (a: number, b: number) => void;
export const wasmgame_setSemanticThreshold: (a: number, b: number) => void;
export const wasmgame_getSemanticThreshold: (a: number) => number;
export const wasmgame_getCardSemanticScore: (a: number, b: number, c: number) => number;
export const wasmgame_cardsMeetingThreshold: (a: number) => number;
export const wasmgame_switchPerspective: (a: number) => [number, number, number];
export const wasmgame_setPerspective: (a: number, b: number) => [number, number];
export const wasmgame_cancelDecision: (a: number) => [number, number, number];
export const wasmgame_dispatch: (a: number, b: any) => [number, number, number];
export const wasmgame_exportSyncCheckpoint: (a: number) => [number, number, number];
export const wasmgame_exportRedactedSyncCheckpoint: (a: number, b: number) => [number, number, number];
export const wasmgame_exportPublicAuditCheckpoint: (a: number) => [number, number, number];
export const wasmgame_importSyncCheckpoint: (a: number, b: any, c: number) => [number, number, number];
export const wasmgame_ziffleKeygen: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleBuildShuffleStep: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleVerifyShuffle: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleBuildRevealToken: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleBuildRevealTokens: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleRevealCard: (a: number, b: any) => [number, number, number];
export const wasmgame_ziffleRevealCards: (a: number, b: any) => [number, number, number];
export const wasmgame_registerManabrewDeckSources: (a: number, b: any) => [number, number, number];
export const wasmgame_validateManabrewMatchConfig: (a: number, b: any) => [number, number, number];
export const wasmgame_startManabrewMatch: (a: number, b: any) => [number, number, number];
export const wasmgame_manabrewView: (a: number, b: number) => [number, number, number];
export const wasmgame_manabrewPublicState: (a: number) => [number, number, number];
export const wasmgame_manabrewRespond: (a: number, b: number, c: number, d: any) => [number, number, number];
export const wasmgame_manabrewApplyDirective: (a: number, b: number, c: any) => [number, number, number];
export const __wbg_wasmgame_free: (a: number, b: number) => void;
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_realloc: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_exn_store: (a: number) => void;
export const __externref_table_alloc: () => number;
export const __wbindgen_externrefs: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_start: () => void;
