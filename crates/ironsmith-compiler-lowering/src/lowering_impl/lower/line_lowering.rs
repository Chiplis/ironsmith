...
use crate::cards::builders::{
    ...
    PlayerAst, PredicateAst, StaticAbilityAst, TriggerFrequencyPredicateAst, TriggerSpec,
};
...
/// The frequency limit the line stated, as a predicate.
///
/// The limit is a fact the front end read off the text, so it travels as a
/// predicate and is bound to a condition by the resolver like any other.
...
fn redirect_amount_replacement(
        ...
        && damage_target(damage).is_some_and(|target| matches!(target.base(), ChooseSpec::Iterated))
    {
        ...
        return Some(vec![redirect_damage_effect(
            damage,
            default_damage.target.clone(),
        )?]);
    }
...
fn attach_cross_line_self_replacement(
    ...
    let replacement_effects = redirect_amount_replacement(
        default_effect,
        &conditional.if_true,
        facts.instead_followup.previous_mixed_damage_recipients,
    )
    ...
    match timing {
        ...
        GiftTimingAst::PermanentEtb => {
            ...
            let parsed = assemble_parsed_triggered_ability(
                ...
                Some(PredicateAst::ThisSpellPaidLabel("Gift".into())),
                None,
...
fn materialize_optional_cost_trigger(
    ...
    let parsed = assemble_parsed_triggered_ability(
        ...
        Vec::new(),
        vec![Zone::Stack],
        Some(PredicateAst::ThisSpellPaidLabel(reference)),
        None,
        prepared.imports.clone(),
    );
