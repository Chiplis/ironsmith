...
impl DocumentCst {
    ...
    pub fn line(&self, line_index: usize) -> Option<&CstLine> {
...
pub fn parse_document_cst(
    ...
    text: &str,
    card_name: &str,
) -> Result<DocumentCst, CardTextError> {
...
fn classify_faces(source: &SourceUnit, lines: &[CstLine]) -> Vec<CstFace> {
    let mut faces = Vec::new();
    let mut face_start = 0usize;
    for (position, line) in lines.iter().enumerate() {
        if !matches!(&line.kind, CstLineKind::FaceSeparator) {
            continue;
        }
...
fn classify_line(
    ...
    let (kind, semantic_content) = if let Some(metadata) = parse_metadata_line(text)? {
        (CstLineKind::Metadata(metadata), Some(trimmed.clone()))
    } else if text == "//" {
        push_node(
            &mut nodes,
            source,
            line_span,
            trimmed.clone(),
            CstNodeKind::FaceSeparator,
        );
        (CstLineKind::FaceSeparator, None)
    } else if is_fully_parenthetical(text) {
        (CstLineKind::ReminderOnly, None)
        ...
        push_node(
            ...
            CstNodeKind::ChapterHeader {
                chapters: chapters.clone(),
            },
        );
        (
            CstLineKind::SagaChapter { chapters },
        ...
        push_node(
            ...
            line_span,
            offset_range(&trimmed, header),
            CstNodeKind::ClassHeader { level },
        );
        (
            CstLineKind::ClassLevel { level },
        ...
        push_node(
            ...
            line_span,
            offset_range(&trimmed, header),
            CstNodeKind::LevelHeader { min, max },
        );
        (
            CstLineKind::LevelBand { min, max },
            body.map(|body| offset_range(&trimmed, body)),
        )
        ...
        push_node(
            ...
            CstNodeKind::ModeMarker(marker),
        );
        (
            CstLineKind::Mode { marker },
            Some(offset_range(&trimmed, body)),
        )
    } else if is_modal_header(text) {
        (CstLineKind::ModalHeader, Some(trimmed.clone()))
        ...
        push_node(
            ...
            CstNodeKind::AbilityWord {
                label: label.to_string(),
            },
        );
        (
            CstLineKind::AbilityWord {
                label: label.to_string(),
            },
            Some(offset_range(&trimmed, body)),
        )
    } else {
        (CstLineKind::RulesText, Some(trimmed.clone()))
    };
...
fn structural_nodes(
    ...
    capture_delimited(authored, '(', ')', |range| {
        ...
        push_node(
            ...
            line_span,
            range,
            CstNodeKind::ReminderText(decision),
        );
    });
    capture_delimited(authored, '{', '}', |range| {
        push_node(&mut nodes, source, line_span, range, CstNodeKind::Symbol);
    });
    ...
    capture_self_references(authored, card_name, |range, surface| {
        push_node(
            ...
            line_span,
            range,
            CstNodeKind::SelfReference(surface),
        );
    });
    capture_punctuation(authored, |range, punctuation| {
        push_node(
            ...
            line_span,
            range,
            CstNodeKind::Punctuation(punctuation),
        );
    });
