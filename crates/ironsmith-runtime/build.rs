...
fn main() {
    ...
    println!("cargo:rustc-check-cfg=cfg(ironsmith_runtime_removed_parser_helper_unit_tests)");
    if std::env::var_os("CARGO_FEATURE_RUNTIME_PARSER_TESTS").is_some() {
        println!("cargo:rustc-cfg=ironsmith_runtime_parser_tests");
        println!("cargo:rustc-cfg=ironsmith_runtime_legacy_parser_unit_tests");
