...
fn effect_exiles_triggering_object(effect: &Effect) -> bool {
...
fn describe_exile_it_then_return_all_to_battlefield(
    first: &Effect,
    second: &Effect,
) -> Option<String> {
...
fn greatest_commander_mana_value_owned_by_iterated(filter: &ObjectFilter) -> bool {
...
fn describe_each_player_may_discard_hand_draw_commander_value(
    ...
    let Value::GreatestManaValue(filter) = &draw.count else {
        return None;
    };
...
fn describe_search_name_conditional_put_then_shuffle(effects: &[&Effect]) -> Option<String> {
...
fn describe_may_cast_target_graveyard_spell_then_exile_replacement(
    effects: &[&Effect],
) -> Option<String> {
    ...
    if !matches!(replacement.target.base(), ChooseSpec::Tagged(tag) if tag == &choose.tag)
    ...
    Some("You may cast target instant or sorcery card from your graveyard. If that spell would be put into your graveyard, exile it instead".to_string())
}
...
fn describe_attack_block_if_able_grant(
...
fn describe_for_players_choose_nonland_put_counter(
    for_players: &crate::effects::ForPlayersEffect,
) -> Option<String> {
...
fn describe_simple_create_token_bundle(effects: &[&Effect]) -> Option<String> {
    if effects.len() < 2 {
        return None;
    }
...
fn is_creature_filter_for_you(filter: &ObjectFilter) -> bool {
    filter.zone == Some(Zone::Battlefield)
...
fn is_creature_card_match_filter(filter: &ObjectFilter) -> bool {
    filter.zone.is_none()
...
fn describe_exile_creatures_consult_that_many_battlefield_shuffle(
    effects: &[&Effect],
) -> Option<String> {
...
fn describe_choose_any_target_players_then_investigate_total_creatures(
    effects: &[&Effect],
) -> Option<String> {
...
fn is_creature_target_opponent_controls_filter(filter: &ObjectFilter) -> bool {
    filter.zone == Some(Zone::Battlefield)
...
fn describe_divided_evenly_x_damage_to_target_opponent_creatures(
...
fn describe_each_player_gain_life_and_draw_pair(effects: &[&Effect]) -> Option<String> {
...
fn describe_search_selection_from_filter_text(
...
fn describe_search_selection_with_cards_preserving_where(selection: &str) -> String {
...
fn describe_for_players_target_return_unless_draw(
    for_players: &crate::effects::ForPlayersEffect,
) -> Option<String> {
...
fn describe_choose_each_basic_land_type_then_destroy(effects: &[&Effect]) -> Option<String> {
...
fn describe_choose_then_put_counter_on_each(effects: &[&Effect]) -> Option<String> {
    let [choose_effect, for_each_effect] = effects else {
        return None;
    };
    let choose = choose_effect.downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
...
pub(super) fn describe_effect_list(effects: &[Effect]) -> String {
    ...
    fn describe_council_vote_winners_exile(effects: &[&Effect]) -> Option<String> {
        ...
        let exile =
            unwrap_tag_wrappers(exile_effect).downcast_ref::<crate::effects::ExileEffect>()?;
    ...
    fn describe_target_players_each_effects(effects: &[&Effect]) -> Option<String> {
        ...
        if count.min != 0 || count.max.is_some() || count.dynamic_x || count.random {
            return None;
        }
        if !matches!(target.as_ref(), ChooseSpec::Target(inner) if matches!(inner.as_ref(), ChooseSpec::Player(PlayerFilter::Any)))
        {
            return None;
        }
        ...
        for effect in per_player_effects {
            ...
            if for_players.filter != PlayerFilter::Target(Box::new(PlayerFilter::Any)) {
                return None;
            }
        ...
        Some(format!("Any number of target players each {joined}"))
    }
    ...
    fn target_only_two_creatures(effect: &Effect) -> Option<(Option<&crate::TagKey>, bool)> {
        let (tag, target_only) =
            if let Some(tagged) = effect.downcast_ref::<crate::effects::TaggedEffect>() {
                (
                    Some(&tagged.tag),
    ...
    fn choose_spec_references_tag_constraint(spec: &ChooseSpec, tag: &crate::TagKey) -> bool {
        match spec {
    ...
    fn describe_two_target_creature_exchange_or_fight(effects: &[&Effect]) -> Option<String> {
    ...
    fn describe_shape_anew_like_bundle(effects: &[&Effect]) -> Option<String> {
        let [
            sacrifice_effect,
            consult_effect,
            move_effect,
    ...
    fn describe_reveal_until_land_put_all_graveyard(effects: &[&Effect]) -> Option<String> {
        ...
        let player = if let Some(target_effect) = target_effect {
            let target_only = target_effect.downcast_ref::<crate::effects::TargetOnlyEffect>()?;
            match &target_only.target {
    ...
    fn describe_tap_for_mana_player_bonus_and_damage(effects: &[&Effect]) -> Option<String> {
    ...
    fn describe_creature_secret_vote_with_default_draw(effects: &[&Effect]) -> Option<String> {
        ...
        let crate::effect::Condition::TaggedObjectMatches(tag, condition_filter) =
            condition.as_ref()
        else {
            return None;
        };
    ...
    fn describe_each_player_choose_creature_destroy_others(effects: &[&Effect]) -> Option<String> {
        let (for_players_effect, destroy_effect) = match effects {
            [for_players_effect, destroy_effect] => (*for_players_effect, *destroy_effect),
            [tag_effect, for_players_effect, destroy_effect]
                if tag_effect
                    .downcast_ref::<crate::effects::TagTriggeringObjectEffect>()
                    .is_some() =>
            {
                (*for_players_effect, *destroy_effect)
            }
            _ => return None,
        };
        ...
        let choose = unwrap_tag_wrappers(choose_effect)
    ...
    fn is_target_opponent_player_filter(player: &PlayerFilter) -> bool {
        matches!(player, PlayerFilter::Target(inner) if matches!(inner.as_ref(), PlayerFilter::Opponent))
    }

    fn is_target_opponent_spec(spec: &ChooseSpec) -> bool {
    ...
    fn is_nonbasic_land_name_filter(filter: &ObjectFilter) -> bool {
        if filter.any_of.len() != 2 {
            return false;
        }
        let excludes_land = filter.any_of.iter().any(|option| {
            option.excluded_card_types == vec![CardType::Land]
                && option.excluded_supertypes.is_empty()
        });
    ...
    fn filter_has_same_name_tag(filter: &ObjectFilter, tag: &TagKey) -> bool {
        filter.tagged_constraints.iter().any(|constraint| {
            constraint.tag == *tag
                && constraint.relation == crate::filter::TaggedOpbjectRelation::SameNameAsTagged
        })
    ...
    fn render_necromentia_shape(effects: &[&Effect]) -> Option<String> {
        ...
        let choose_name =
            choose_name_effect.downcast_ref::<crate::effects::ChooseCardNameEffect>()?;
        ...
        let create = unwrap_tag_wrappers(create_effect)
    ...
    fn render_optional_draw_then_sylvan_card_choice(effects: &[&Effect]) -> Option<String> {
        ...
        if draw.player != PlayerFilter::You || draw.count != Value::Fixed(2) {
            return None;
        }
        ...
        let move_to_zone = unless.effects[0].downcast_ref::<crate::effects::MoveToZoneEffect>()?;
        if move_to_zone.zone != Zone::Library
            || !move_to_zone.to_top
        ...
        let lose_life = unless.alternative[0].downcast_ref::<crate::effects::LoseLifeEffect>()?;
    ...
    fn describe_choose_then_put_sticker(
        choose: &crate::effects::ChooseObjectsEffect,
    ...
    fn downcast_set_base_power_toughness<'a>(
        effect: &'a Effect,
    ...
    fn downcast_destroy<'a>(effect: &'a Effect) -> Option<&'a crate::effects::DestroyEffect> {
    ...
    fn downcast_destroy_no_regeneration<'a>(
        effect: &'a Effect,
    ...
    fn downcast_return_from_graveyard_to_battlefield<'a>(
        effect: &'a Effect,
    ...
    fn downcast_mill<'a>(effect: &'a Effect) -> Option<&'a crate::effects::MillEffect> {
        unwrap_tag_wrappers(effect).downcast_ref::<crate::effects::MillEffect>()
    }

    fn apply_removes_all_abilities(apply: &crate::effects::ApplyContinuousEffect) -> bool {
    ...
    fn each_subject_from_filter(filter: &crate::filter::ObjectFilter) -> String {
    ...
    fn render_remove_abilities_then_destroy_matching_creatures(
    ...
    fn describe_exile_split_pile_opponent_choice_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        let alt_selected = unwrap_tag_wrappers(alt_selected)
            .downcast_ref::<crate::effects::ReturnToHandEffect>()?;
    ...
    fn describe_creature_pile_destroy_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn describe_graveyard_creature_pile_exile_return_bundle(
        filtered: &[&Effect],
    ) -> Option<String> {
    ...
    fn describe_damage_and_die_replacement_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn describe_reveal_hand_exile_same_name_search_bundle(filtered: &[&Effect]) -> Option<String> {
        let [
            look_effect,
            choose_effect,
            exile_effect,
        ...
        let [search_effect] = for_each_search.effects.as_slice() else {
            return None;
        };
        let search = search_effect.downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
        let for_each_move =
    ...
    fn describe_reveal_hand_choose_shuffle_into_library_bundle(
        filtered: &[&Effect],
    ) -> Option<String> {
    ...
    fn describe_tempting_offer_creature_return_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        let choose_self =
            choose_self_effect.downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
        ...
        if may.decider.is_some()
            || choose_opp.chooser != PlayerFilter::You
        ...
        let choose_bonus =
            choose_bonus_effect.downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
    ...
    fn describe_mill_return_land_else_counter_bundle(filtered: &[&Effect]) -> Option<String> {
        if let [mill_effect, choose_effect, for_each_effect, if_effect] = filtered {
            ...
            let put_counter =
                if_effect.then[0].downcast_ref::<crate::effects::PutCountersEffect>()?;
    ...
    fn describe_dynamic_pt_token_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn describe_create_token_then_set_base_pt_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        if set_pt.power != set_pt.toughness || matches!(set_pt.power, Value::Fixed(_)) {
            return None;
        }
    ...
    fn describe_each_player_choose_unselected_bounce_bundle(
        filtered: &[&Effect],
    ) -> Option<String> {
        ...
        let choose =
            for_players.effects[0].downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
    ...
    fn describe_grant_haste_and_unblockable_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        let has_haste = apply.modification.as_ref().is_some_and(|modification| {
            matches!(modification, crate::continuous::Modification::AddAbility(ability) if ability.id() == crate::static_abilities::StaticAbilityId::Haste)
        }) || apply.additional_modifications.iter().any(|modification| {
            matches!(modification, crate::continuous::Modification::AddAbility(ability) if ability.id() == crate::static_abilities::StaticAbilityId::Haste)
        });
        if !has_haste {
    ...
    fn describe_return_creature_mana_value_scry_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn describe_exchange_control_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        if let (Some(player_one), Some(player_two)) = (choose_player_one, choose_player_two) {
            if choose_one.filter.controller
    ...
    fn describe_graveyard_mana_ladder_return_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        for (idx, choose) in chooses.iter().enumerate() {
            if choose.chooser != PlayerFilter::You
                ...
                || choose.filter.mana_value
                    != Some(crate::filter::Comparison::Equal((idx + 1) as i32))
            {
                return None;
            }
        }
    ...
    fn describe_random_hand_reveal_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn describe_choose_then_reveal_from_hand_bundle(filtered: &[&Effect]) -> Option<String> {
        let [choose_effect, reveal_effect] = filtered else {
            return None;
        };
        let choose = choose_effect.downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
    ...
    fn describe_choose_reveal_from_hand_then_reflexive_bundle(
        filtered: &[&Effect],
    ) -> Option<String> {
    ...
    fn describe_self_unblockable_bundle(filtered: &[&Effect]) -> Option<String> {
        let [apply_effect, choose_effect, cant_effect] = filtered else {
            return None;
        };
        let apply = apply_continuous_for_compaction(apply_effect)?;
    ...
    fn describe_tap_freeze_bundle(filtered: &[&Effect]) -> Option<String> {
        ...
        let target_text = describe_choose_spec(&tap.target);
    ...
    fn describe_reveal_top_to_hand_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    fn filter_is_tagged_as(filter: &ObjectFilter, tag: &str) -> bool {
        filter.tagged_constraints.iter().any(|constraint| {
            constraint.relation == crate::filter::TaggedOpbjectRelation::IsTaggedObject
                && constraint.tag.as_str() == tag
        })
    ...
    fn search_origin_for_divvy(search: &crate::effects::ChooseObjectsEffect) -> Option<String> {
        Some(describe_search_origin_zones(search)?.replace(" and/or ", " and "))
    }

    fn render_search_reveal_opponent_choose_rest_bundle(filtered: &[&Effect]) -> Option<String> {
    ...
    if let Some(compact) = describe_grant_haste_and_unblockable_bundle(&filtered) {
        return compact;
    }
    ...
    fn describe_discard_then_for_each_discarded(
    ...
    fn describe_ticket_then_may_put_sticker(
    ...
    while idx < filtered.len() {
        ...
        fn optional_nonland_permanent_choice<'a>(
            effect: &'a Effect,
            zone: Zone,
            tag: Option<&str>,
        ) -> Option<&'a str> {
        ...
        fn choose_targets_schedule_trigger(
            choose: &crate::effects::ChooseObjectsEffect,
        ...
        fn consult_reveal_put_battlefield_then_bottom_selection(
            for_each: &crate::effects::ForEachTaggedEffect,
        ) -> Option<String> {
        ...
        fn tagged_exile_any_number_target_creatures(effect: &Effect) -> Option<&str> {
            let tagged = effect.downcast_ref::<crate::effects::TaggedEffect>()?;
            let exile = tagged
                .effect
                .downcast_ref::<crate::effects::ExileEffect>()?;
        ...
        fn render_consult_reveal_put_battlefield_rest_graveyard(
            ...
            let move_remainder =
                graveyard_effect.downcast_ref::<crate::effects::MoveToZoneEffect>()?;
        ...
        fn render_consult_reveal_put_all_revealed_into_hand(effects: &[&Effect]) -> Option<String> {
            if effects.len() != 2 {
                return None;
            }
        ...
        fn render_each_player_exile_top_then_cast_any_number(
            effects: &[&Effect],
        ) -> Option<String> {
        ...
        fn render_sacrifice_then_consult_reveal_put_battlefield_rest_bottom(
            ...
            let sacrificed_subject = if choose.filter.card_types
            ...
            let stop_text = match &consult.stop_rule {
                ...
                crate::effects::ConsultTopOfLibraryStopRule::MatchCount(Value::EffectValue(_)) => {
                    format!(
        ...
        fn render_repeated_named_searches_to_hand(effects: &[&Effect]) -> Option<(String, usize)> {
        ...
        fn describe_single_search_selection_with_cards(
            choose: &crate::effects::ChooseObjectsEffect,
        ) -> String {
        ...
        fn render_single_named_search_to_hand_with_conditional_shuffle(
            effects: &[&Effect],
        ) -> Option<(String, usize)> {
            let choose = effects
                .first()?
                ...
                .and_then(|effect| effect.downcast_ref::<crate::effects::IfEffect>())
            {
                ...
                shuffle_text = if player == "you" {
                    ". If you searched your library this way, shuffle".to_string()
                } else {
                    format!(
        ...
        fn keyword_label_from_static_ability(
        ...
        fn choose_references_revealed_tag(
            choose: &crate::effects::ChooseObjectsEffect,
        ...
        fn revealed_choice_label(choose: &crate::effects::ChooseObjectsEffect) -> Option<String> {
        ...
        fn conditional_revealed_choice<'a>(
            ...
            let choose =
                conditional.if_true[0].downcast_ref::<crate::effects::ChooseObjectsEffect>()?;
        ...
        fn for_each_returns_iterated_to_hand(effect: &Effect, tag: &crate::TagKey) -> bool {
            ...
            if let Some(return_to_hand) =
                effect.downcast_ref::<crate::effects::ReturnToHandEffect>()
            {
        ...
        fn for_each_conditionally_returns_iterated_to_hand(
            effect: &Effect,
            tag: &crate::TagKey,
        ) -> bool {
            let Some((_, for_each)) = for_each_tagged_for_compaction(effect) else {
            ...
            let Some(conditional) =
                for_each.effects[0].downcast_ref::<crate::effects::ConditionalEffect>()
            else {
                return false;
            };
            if !conditional.if_true.is_empty() || conditional.if_false.len() != 1 {
        ...
        fn for_each_moves_any_remainder_to_zone(
            effect: &Effect,
            tag: &crate::TagKey,
            zone: Zone,
        ) -> bool {
        ...
        fn put_battlefield_uses_tag(effect: &Effect, tag: &crate::TagKey) -> bool {
            let effect = unwrap_tag_wrappers(effect);
        ...
        fn render_look_reveal_repeated_choices(effects: &[&Effect]) -> Option<(String, usize)> {
            let look = effects
                .first()?
            ...
            let reveal = effects
                .get(1)?
                .downcast_ref::<crate::effects::RevealTaggedEffect>()?;
            if look.player != PlayerFilter::You || reveal.tag != look.tag {
                return None;
            }
            ...
            while idx < effects.len() {
                ...
                if let Some(conditional) =
                    effects[idx].downcast_ref::<crate::effects::ConditionalEffect>()
                {
                    if let Some((label, tag)) =
            ...
            let labels_are_card_types = labels
                .iter()
                .all(|label| label.ends_with(" card") && !label.contains(" with "));
            let labels_are_keyword_cards =
                labels.iter().all(|label| label.starts_with("card with "));
                ...
                && remainder.player == PlayerFilter::You
            {
                ...
                let choice_text = if labels_are_card_types {
                ...
                return Some((
                    format!(
                        "{look_text}. Reveal them. {choice_text}. Put the rest on the bottom of your library{order_text}"
                    ),
                    idx + 2,
                ));
            }
            ...
            if labels_are_keyword_cards
                ...
                && (for_each_returns_iterated_to_hand(effects[idx + 2], &chosen_tag)
                    || for_each_conditionally_returns_iterated_to_hand(
                        effects[idx + 2],
                        &chosen_tag,
                    ))
                && for_each_moves_any_remainder_to_zone(
                    effects[idx + 3],
                    &look.tag,
                    Zone::Graveyard,
                )
        ...
        fn count_override_look_choice(
            ...
            Some(exact_count as u32)
        }

        fn render_look_at_top_count_override_with_conditional(
            effects: &[&Effect],
        ) -> Option<(String, usize)> {
            ...
            let kicked_count = count_override_look_choice(look_at_top, &conditional.if_true)?;
            let normal_count = count_override_look_choice(look_at_top, &conditional.if_false)?;
        ...
        fn render_reveal_top_choose_to_hand(effects: &[&Effect]) -> Option<(String, usize)> {
            let reveal = effects
                .first()?
        ...
        if idx + 2 < filtered.len()
            && let Some(rendered) =
                describe_grant_haste_and_unblockable_bundle(&filtered[idx..idx + 3])
        {
            parts.push(rendered);
            idx += 3;
            continue;
            ...
            && if_effect.else_.is_empty()
        {
            ...
            if !setup.is_empty() && !followup.is_empty() {
                let condition = match may.decider.as_ref() {
...
fn move_to_zone_for_transform_compaction(
    effect: &Effect,
) -> Option<(
...
fn transform_targets_returned_object(
    move_back: &crate::effects::MoveToZoneEffect,
    ...
    move_back.target == transform.target
}

pub(super) fn describe_return_then_transform(
    return_effect: &Effect,
    transform_effect: &Effect,
) -> Option<String> {
...
pub(super) fn describe_inline_ability_with_self_subject(
    ability: &Ability,
    self_subject: &str,
) -> String {
...
pub(super) fn granted_ability_self_subject_for_choose_spec(spec: &ChooseSpec) -> &'static str {
    match spec.base() {
        ChooseSpec::Object(filter) | ChooseSpec::All(filter) => {
...
mod tests {
    ...
    #[test]
    fn describe_effect_list_compacts_choose_then_put_counter_on_each() {
        let choose = crate::effects::ChooseObjectsEffect::new(
    ...
    #[test]
    fn describe_effect_list_compacts_two_optional_nonland_exile_targets() {
    ...
    #[test]
    fn describe_source_exiled_cards_return_to_hand_uses_exiled_cards() {
    ...
    #[test]
    fn describe_effect_list_compacts_optional_draw_then_drawn_card_choice() {
        let tag = TagKey::from("__it__");
    ...
    #[test]
    fn describe_effect_list_compacts_chaos_mutation_shape() {
    ...
    #[test]
    fn describe_effect_list_compacts_nonbasic_name_multi_zone_search_reward() {
    ...
    #[test]
    fn describe_draw_for_each_tagged_matching_preserves_hand_origin() {
    ...
    #[test]
    fn describe_shuffle_objects_into_library_uses_owner_and_graveyard_wording() {
...
fn describe_payment_each_value(value: &Value) -> String {
    match value {
...
fn describe_cost_component_parts(costs: &[crate::costs::Cost]) -> Vec<String> {
    let mut parts = Vec::new();
    let mut idx = 0usize;
    while idx < costs.len() {
...
fn describe_exile_source_and_named_artifact_costs(
...
fn named_artifact_exile_cost_name(
    choose: &crate::effects::ChooseObjectsEffect,
...
fn title_case_named_card_selection(selection: &str) -> String {
...
pub(super) fn describe_for_each_put_counters_then_untap(
    for_each: &crate::effects::ForEachObject,
) -> Option<String> {
...
fn describe_choose_creature_type_then_must_attack(
    choose: &crate::effects::ChooseCreatureTypeEffect,
    followup: &Effect,
) -> Option<String> {
    if !choose.excluded_subtypes.is_empty() || !matches!(choose.chooser, PlayerFilter::You) {
...
fn describe_for_players_may_choose_then_move_to_battlefield(
    for_players: &crate::effects::ForPlayersEffect,
) -> Option<String> {
    let subject = match for_players.filter {
        PlayerFilter::Any => "Each player",
...
fn describe_runtime_choice_count(choose: &crate::effects::ChooseObjectsEffect) -> Option<String> {
...
fn describe_runtime_choice_where_clause(
    choose: &crate::effects::ChooseObjectsEffect,
) -> Option<String> {
...
fn describe_choose_exile_then_put_counter(
    choose: &crate::effects::ChooseObjectsEffect,
    exile: &crate::effects::ExileEffect,
...
pub(super) fn return_to_hand_uses_chosen_tag(
...
pub(super) fn describe_target_player_choose_half_then_return_to_hand(
    choose: &crate::effects::ChooseObjectsEffect,
    return_to_hand: &crate::effects::ReturnToHandEffect,
) -> Option<String> {
    if choose_primary_zone(choose) != Some(Zone::Battlefield)
...
fn looked_card_choice_filter_is_plain_remainder(
    choose: &crate::effects::ChooseObjectsEffect,
    looked_tag: &str,
    excluded_tags: &[&str],
) -> bool {
...
pub(super) fn describe_look_at_top_exile_face_down_then_play_while_exiled(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    exile: &crate::effects::ExileEffect,
    grant: &crate::effects::GrantPlayTaggedEffect,
) -> Option<String> {
...
fn condition_matches_tagged_object_membership(
    condition: &crate::effect::Condition,
    tag: &str,
) -> bool {
    matches!(
        ...
        crate::effect::Condition::PlayerTaggedObjectMatches {
            ...
            filter,
            ..
        } if condition_tag.as_str() == tag && filter_is_membership_test_for_chosen(filter, tag)
    ) || matches!(
        condition,
...
fn effect_moves_iterated_to_zone(effect: &Effect, zone: Zone) -> bool {
    matches!(
        effect.downcast_ref::<crate::effects::MoveToZoneEffect>(),
        Some(move_to_zone)
            if move_to_zone.zone == zone
...
fn effect_moves_iterated_if_not_tagged_to_zone(
    effect: &Effect,
    excluded_tags: &[&str],
    zone: Zone,
) -> bool {
...
fn for_each_moves_unselected_from_any_to_zone(
    for_each: &crate::effects::ForEachTaggedEffect,
    looked_tag: &str,
    excluded_tags: &[&str],
    zone: Zone,
...
fn describe_any_number_filter_from_looked_cards(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    choose: &crate::effects::ChooseObjectsEffect,
) -> Option<String> {
    if choose_primary_zone(choose) != Some(Zone::Library)
    ...
    Some(pluralize_noun_phrase(strip_leading_article(&card_desc)))
}

fn describe_choose_filter_from_looked_cards_with_ignored_tags(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    choose: &crate::effects::ChooseObjectsEffect,
    ignored_tags: &[&str],
) -> Option<String> {
...
fn normalize_looked_card_filter_description(filter: &ObjectFilter, card_desc: &str) -> String {
...
fn unwrap_tag_wrapped_effect<'a>(effect: &'a Effect) -> &'a Effect {
    if let Some(tag_all) = effect.downcast_ref::<crate::effects::TagAllEffect>() {
...
fn tag_all_wrapper_tag_for_effect<'a>(effect: &'a Effect) -> Option<&'a str> {
    if let Some(tag_all) = effect.downcast_ref::<crate::effects::TagAllEffect>() {
...
fn land_move_from_chosen_effect(effect: &Effect, tag: &str) -> Option<bool> {
...
pub(super) fn describe_look_at_top_then_put_onto_battlefield_and_into_hand_rest_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
...
fn for_each_puts_tag_onto_battlefield(
    for_each: &crate::effects::ForEachTaggedEffect,
    tag: &str,
) -> Option<bool> {
    if for_each.tag.as_str() != tag || for_each.effects.len() != 1 {
...
pub(super) fn describe_look_at_top_then_may_put_battlefield_else_hand_rest_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
...
fn for_each_moves_tag_to_library_top(
    for_each: &crate::effects::ForEachTaggedEffect,
    tag: &str,
) -> bool {
    fn iterated_or_tagged(spec: &ChooseSpec, tag: &str) -> bool {
...
pub(super) fn describe_look_at_top_then_reveal_put_on_top_rest_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    choose: &crate::effects::ChooseObjectsEffect,
...
fn for_each_moves_tag_to_public_zone(
    for_each: &crate::effects::ForEachTaggedEffect,
    tag: &str,
) -> Option<(Zone, bool)> {
    if for_each.tag.as_str() != tag || for_each.effects.len() != 1 {
...
fn describe_look_at_top_then_put_any_matching_to_zone_rest_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    reveal_top: Option<&crate::effects::RevealTaggedEffect>,
    choose: &crate::effects::ChooseObjectsEffect,
    ...
    let (count_text, noun, count_where_clause) =
        describe_top_count_noun_and_where_clause(&look_at_top.count);
    let put_prefix = if choose.filter.distinct_powers {
        if choose.chooser == PlayerFilter::You {
            "Put".to_string()
        } else {
            format!(
...
pub(super) fn describe_look_at_top_then_put_one_hand_other_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    choose: &crate::effects::ChooseObjectsEffect,
...
pub(super) fn describe_look_at_top_then_put_chosen_hand_rest_bottom_from_for_each(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    choose: &crate::effects::ChooseObjectsEffect,
    move_to_hand: &crate::effects::ForEachTaggedEffect,
    rest: &crate::effects::ForEachTaggedEffect,
...
pub(super) fn describe_look_at_top_then_put_into_hand_rest_graveyard(
    ...
    let actor_prefix = if exact_choice {
        String::new()
    } else if choose.chooser == PlayerFilter::You {
        "You may ".to_string()
    } else {
...
pub(super) fn describe_look_at_top_then_reveal_put_matching_into_hand_rest_bottom(
    look_at_top: &crate::effects::LookAtTopCardsEffect,
    reveal_tagged: &crate::effects::RevealTaggedEffect,
    tag_matching: &crate::effects::TagMatchingObjectsEffect,
...
fn describe_draw_lose_life_get_energy(
    draw: &crate::effects::DrawCardsEffect,
    lose: &crate::effects::LoseLifeEffect,
...
pub(super) fn describe_may_search_reveal_shuffle_then_conditional_move(
    may: &crate::effects::MayEffect,
    conditional: &crate::effects::ConditionalEffect,
) -> Option<String> {
    fn downcast_move_to_zone<'a>(
    ...
    fn move_uses_tag(move_to_zone: &crate::effects::MoveToZoneEffect, tag: &str) -> bool {
        matches!(move_to_zone.target.base(), ChooseSpec::Tagged(found) if found.as_str() == tag)
    }
    ...
    if true_move.zone != Zone::Hand
        ...
        || !false_move.to_top
        || !move_uses_tag(true_move, choose.tag.as_str())
        || !move_uses_tag(false_move, choose.tag.as_str())
    {
        return None;
    }
...
pub(super) fn describe_search_reveal_named_conditional_move_then_shuffle(
    choose: &crate::effects::ChooseObjectsEffect,
...
pub(super) fn describe_may_search_choose_for_each_with_shuffle(
    ...
    let compact = if choose_search_zones(&choose).is_some_and(|zones| zones.len() > 1)
        && compact.ends_with(", then shuffle")
    {
...
fn describe_search_choose_then_move(
    choose: &crate::effects::ChooseObjectsEffect,
...
fn describe_search_choose_then_exile(
    choose: &crate::effects::ChooseObjectsEffect,
    reveal: Option<&crate::effects::RevealTaggedEffect>,
    exile: &crate::effects::ExileEffect,
    shuffle: Option<&crate::effects::ShuffleLibraryEffect>,
...
fn describe_search_choose_then_return_to_hand(
    choose: &crate::effects::ChooseObjectsEffect,
    reveal: Option<&crate::effects::RevealTaggedEffect>,
    return_to_hand: &crate::effects::ReturnToHandEffect,
    shuffle: Option<&crate::effects::ShuffleLibraryEffect>,
...
fn search_color_count_references_sacrifice_cost(
    choose: &crate::effects::ChooseObjectsEffect,
) -> bool {
...
fn describe_search_face_down_exile_shuffle_conditional_cast_else_hand(
    choose: &crate::effects::ChooseObjectsEffect,
    exile: &crate::effects::ExileEffect,
    shuffle: &crate::effects::ShuffleLibraryEffect,
    conditional: &crate::effects::ConditionalEffect,
...
fn describe_for_each_same_name_search_to_battlefield(
    for_each: &crate::effects::ForEachObject,
    move_effect: &Effect,
    shuffle: &crate::effects::ShuffleLibraryEffect,
) -> Option<String> {
        ...
        if simple_damage_filter && matches!(prevent_all.until, Until::EndOfTurn) {
            if prevent_all.damage_filter.combat_only {
                if matches!(prevent_all.target, crate::prevention::PreventionTarget::All) {
                    return "Prevent all combat damage this turn".to_string();
                }
                return format!(
...
fn describe_structural_cycling_keyword(
    ability: &Ability,
    ...
    let cost_text = keyword_base_cost_text(activated.mana_cost.costs(), |cost| {
        is_discard_this_card_cost(cost) || is_cycle_event_cost(cost)
    })?;
    let rendered = keywords
        .into_iter()
        .map(|keyword| {
            if keyword == "Cycling" && cost_text.to_ascii_lowercase().starts_with("pay ") {
                format!("Cycling—{cost_text}")
            } else {
...
fn cycling_keywords_for_search_filter(filter: &ObjectFilter) -> Vec<String> {
...
fn describe_structural_equip_keyword(
    activated: &crate::ability::ActivatedAbility,
) -> Option<String> {
...
fn describe_structural_crew_keyword(
    activated: &crate::ability::ActivatedAbility,
) -> Option<String> {
...
fn describe_structural_cumulative_upkeep_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
...
fn cumulative_upkeep_payment_text(payment: &[Effect]) -> Option<String> {
...
fn cumulative_upkeep_chosen_put_counters_text(payment: &[Effect]) -> Option<String> {
...
fn cumulative_upkeep_chosen_sacrifice_text(payment: &[Effect]) -> Option<String> {
...
fn cumulative_upkeep_sacrifice_text(filter: &ObjectFilter, count: &Value) -> Option<String> {
...
fn cumulative_counter_phrase(put_counters: &crate::effects::PutCountersEffect) -> Option<String> {
...
fn cumulative_upkeep_put_counters_text(
...
fn describe_structural_riot_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
        || triggered.trigger.display() != "When this creature enters"
        || !triggered
            .trigger
            .downcast_ref::<crate::triggers::ZoneChangeTrigger>()
            .is_some_and(|trigger| trigger.this_object)
    {
        return None;
    }
...
fn is_plus_one_counter_on_source(effect: &Effect) -> bool {
    effect
        .downcast_ref::<crate::effects::PutCountersEffect>()
        .is_some_and(|put| {
...
fn apply_continuous_adds_static_ability(
    apply: &crate::effects::ApplyContinuousEffect,
...
fn describe_structural_exalted_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
...
fn describe_structural_afterlife_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
...
fn describe_structural_toxic_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
...
fn describe_structural_storm_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
...
fn describe_structural_mobilize_keyword(
    triggered: &crate::ability::TriggeredAbility,
) -> Option<String> {
    if triggered.intervening_if.is_some()
        || !triggered.choices.is_empty()
...
fn is_discard_this_card_cost(cost: &crate::costs::Cost) -> bool {
...
fn is_cycle_event_cost(cost: &crate::costs::Cost) -> bool {
    cost.effect_ref()
...
fn describe_annihilator_keyword(triggered: &crate::ability::TriggeredAbility) -> Option<String> {
    if triggered.intervening_if.is_some() || !triggered.choices.is_empty() {
        return None;
    }
    if !triggered
