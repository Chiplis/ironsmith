...
fn new_game() -> GameState {
    ...
    for player in [ALICE, BOB] {
        for i in 0..6 {
            game.create_object_from_definition(&filler(&format!("Library {i}")), player, Zone::Library);
        }
    }
    game
}
...
impl Script {
    fn targeting(targets: Vec<Vec<Target>>) -> Self {
        Self { targets: targets.into(), ..Self::default() }
    }
}
...
impl DecisionMaker for Script {
    ...
    fn decide_objects(&mut self, _game: &GameState, ctx: &SelectObjectsContext) -> Vec<ObjectId> {
        ...
        ctx.candidates.iter().filter(|c| c.legal).map(|c| c.id).take(wanted).collect()
    }

    fn decide_options(&mut self, _game: &GameState, ctx: &SelectOptionsContext) -> Vec<usize> {
        self.option_prompts.push((
            ctx.player,
            ctx.options.iter().map(|option| option.description.clone()).collect(),
        ));
    ...
    fn decide_targets(&mut self, _game: &GameState, ctx: &TargetsContext) -> Vec<Target> {
        ...
        ctx.requirements
            ...
            .flat_map(|requirement| {
                let count = requirement.min_targets.max(1).min(requirement.legal_targets.len());
...
fn cast(game: &mut GameState, queue: &mut TriggerQueue, player: PlayerId, spell: ObjectId, dm: &mut Script) {
...
mod b1_hexproof_spell_can_be_countered {
    ...
    fn counterspell_targets_a_hexproof_creature_spell() {
        ...
        let counterspell = game.create_object_from_definition(&card("Counterspell"), ALICE, Zone::Hand);
        game.player_mut(BOB).unwrap().mana_pool.add(ManaSymbol::Green, 1);
        ...
        assert!(dm.target_prompts[0].1.contains(&Target::Object(scout_spell)));
...
mod b2_ward_only_on_battlefield {
    ...
    fn graveyard_ward_card_does_not_tax_a_graveyard_targeting_spell() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let terror = game.create_object_from_definition(&card("Tolarian Terror"), BOB, Zone::Graveyard);
        ...
        cast(&mut game, &mut queue, ALICE, cremate, &mut Script::targeting(vec![vec![Target::Object(terror)]]));
    ...
    fn countering_a_ward_creature_spell_is_not_taxed() {
        ...
        let stable = game.object(terror).unwrap().stable_id;
        let counterspell = game.create_object_from_definition(&card("Counterspell"), ALICE, Zone::Hand);
        game.player_mut(BOB).unwrap().mana_pool.add(ManaSymbol::Blue, 7);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Blue, 2);
        cast(&mut game, &mut queue, BOB, terror, &mut Script::default());
        let spell = game.stack[0].object_id;
        cast(&mut game, &mut queue, ALICE, counterspell, &mut Script::targeting(vec![vec![Target::Object(spell)]]));
...
mod b3_ward_respects_cant_be_countered {
    ...
    fn abrupt_decay_resolves_through_unpaid_ward() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let bear = game.create_object_from_definition(&ward_bear("Ward {2}"), BOB, Zone::Battlefield);
        ...
        cast(&mut game, &mut queue, ALICE, decay, &mut Script::targeting(vec![vec![Target::Object(bear)]]));
...
mod b4_ward_is_a_trigger {
    ...
    fn unpaid_ward_trigger_counters_the_targeting_spell() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let terror = game.create_object_from_definition(&card("Tolarian Terror"), BOB, Zone::Battlefield);
        ...
        cast(&mut game, &mut queue, ALICE, shock, &mut Script::targeting(vec![vec![Target::Object(terror)]]));
        assert_eq!(game.stack.len(), 2);
        let ward = game.stack.last().unwrap();
        assert!(ward.is_ability, "ward is a triggered ability above the spell");
    ...
    fn each_ward_instance_triggers() {
        ...
        let shock = game.create_object_from_definition(&card("Shock"), ALICE, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Red, 1);
        cast(&mut game, &mut queue, ALICE, shock, &mut Script::targeting(vec![vec![Target::Object(bear)]]));
...
mod b5_battle_timing {
    ...
    fn battle_is_not_castable_on_an_opponents_turn() {
        let mut game = new_game();
        let invasion = game.create_object_from_definition(&card("Invasion of Zendikar"), ALICE, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Green, 4);
        assert!(find_cast(&game, ALICE, invasion).is_some(), "main phase, empty stack");
        game.turn.active_player = BOB;
        game.turn.priority_player = Some(ALICE);
...
mod b6_player_protection_drops_curses {
    ...
    fn teferis_protection_removes_an_attached_curse() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let curse = game.create_object_from_definition(&card("Curse of Death's Hold"), BOB, Zone::Battlefield);
        assert!(game.attach_object_to_target(curse, ironsmith::object::AttachmentTarget::Player(ALICE)));
        let protection = game.create_object_from_definition(&card("Teferi's Protection"), ALICE, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::White, 3);
        cast(&mut game, &mut queue, ALICE, protection, &mut Script::default());
        resolve_all(&mut game, &mut queue, &mut Script::default());
...
mod b8_uncast_aura_respects_protection {
    ...
    fn pacifism_stays_in_graveyard_when_only_a_pro_white_creature_exists() {
        ...
        let pacifism = game.create_object_from_definition(&card("Pacifism"), ALICE, Zone::Graveyard);
        let stable = game.object(pacifism).unwrap().stable_id;
        let _ = game.move_object_with_etb_processing_with_dm(pacifism, Zone::Battlefield, &mut Script::default());
        assert_eq!(zone_of_card(&game, stable), Some(Zone::Graveyard));
    }
    ...
    fn pacifism_attaches_to_the_creature_without_protection() {
        let mut game = new_game();
        game.create_object_from_definition(&card("Black Knight"), BOB, Zone::Battlefield);
        let bears = game.create_object_from_definition(&vanilla("Grizzly Bears", 2, 2), BOB, Zone::Battlefield);
        let pacifism = game.create_object_from_definition(&card("Pacifism"), ALICE, Zone::Graveyard);
        let stable = game.object(pacifism).unwrap().stable_id;
        let _ = game.move_object_with_etb_processing_with_dm(pacifism, Zone::Battlefield, &mut Script::default());
...
mod b9_hexproof_from_permission {
    ...
    fn disfigure_can_target_harbinger(spotlight: bool) -> bool {
        let mut game = new_game();
        if spotlight {
            game.create_object_from_definition(&card("Glaring Spotlight"), ALICE, Zone::Battlefield);
        }
...
mod e1_03_fight_with_illegal_target {
    ...
    fn prey_upon_does_nothing_when_its_target_gains_hexproof() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let mine = game.create_object_from_definition(&vanilla("Alice Bear", 3, 3), ALICE, Zone::Battlefield);
        let targeted = game.create_object_from_definition(&vanilla("Bob Target", 2, 2), BOB, Zone::Battlefield);
        let other = game.create_object_from_definition(&vanilla("Bob Other", 2, 2), BOB, Zone::Battlefield);
        let prey = game.create_object_from_definition(&card("Prey Upon"), ALICE, Zone::Hand);
        let defense = game.create_object_from_definition(&card("Blossoming Defense"), BOB, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Green, 1);
        game.player_mut(BOB).unwrap().mana_pool.add(ManaSymbol::Green, 1);
        cast(
            &mut game,
        ...
        cast(&mut game, &mut queue, BOB, defense, &mut Script::targeting(vec![vec![Target::Object(targeted)]]));
        resolve_all(&mut game, &mut queue, &mut Script::default());
        for id in [mine, targeted, other] {
            assert_eq!(game.damage_on(id), 0, "{:?}", game.object(id).map(|o| o.name.to_string()));
        }
    }
}
...
mod e1_06_stack_ability_identity {
    ...
    fn stifle_counters_a_storm_trigger_above_its_spell() {
        ...
        cast(&mut game, &mut queue, ALICE, first, &mut Script::targeting(vec![vec![Target::Player(BOB)]]));
        resolve_all(&mut game, &mut queue, &mut Script::default());
        game.turn.priority_player = Some(ALICE);
        cast(&mut game, &mut queue, ALICE, grapeshot, &mut Script::targeting(vec![vec![Target::Player(BOB)]]));
        ...
        resolve_all(&mut game, &mut queue, &mut Script::targeting(vec![vec![Target::Player(BOB)]; 4]));
    ...
    fn stifle_counters_exactly_the_chosen_one_of_two_abilities_from_one_source() {
        ...
        assert!(offered.contains(&Target::Object(top)) && offered.contains(&Target::Object(bottom)));
        resolve_top(&mut game, &mut queue, &mut Script::default());
        assert_eq!(game.stack.len(), 1);
        assert_eq!(game.stack[0].ability_id, Some(bottom), "the other ability remains");
        resolve_all(&mut game, &mut queue, &mut Script::default());
...
mod e1_08_exchange_rechecks_targets {
    ...
    fn switcheroo_does_nothing_when_one_target_gains_hexproof() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let mine = game.create_object_from_definition(&vanilla("Alice Bear", 2, 2), ALICE, Zone::Battlefield);
        let theirs = game.create_object_from_definition(&vanilla("Bob Bear", 2, 2), BOB, Zone::Battlefield);
        let switcheroo = game.create_object_from_definition(&card("Switcheroo"), ALICE, Zone::Hand);
        let defense = game.create_object_from_definition(&card("Blossoming Defense"), BOB, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Blue, 5);
        game.player_mut(BOB).unwrap().mana_pool.add(ManaSymbol::Green, 1);
        cast(
            &mut game,
            &mut queue,
        ...
        cast(&mut game, &mut queue, BOB, defense, &mut Script::targeting(vec![vec![Target::Object(theirs)]]));
        resolve_all(&mut game, &mut queue, &mut Script::default());
...
mod c4_replacement_order_for_stolen_entry {
    ...
    fn reanimating_player_orders_counter_replacements() {
        ...
        let feeder = game.create_object_from_definition(&card("Spike Feeder"), BOB, Zone::Graveyard);
        ...
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Black, 1);
        cast(&mut game, &mut queue, ALICE, reanimate, &mut Script::targeting(vec![vec![Target::Object(feeder)]]));
        ...
        let order_prompts: Vec<_> = dm
            .option_prompts
            .iter()
            .filter(|(_, options)| options.iter().any(|o| o.contains("Hardened Scales") || o.contains("Doubling Season")))
            .collect();
        assert!(!order_prompts.is_empty(), "{:?}", dm.option_prompts);
        assert!(order_prompts.iter().all(|(player, _)| *player == ALICE), "{order_prompts:?}");
    }
}
...
mod c8_legend_rule_simultaneous {
    ...
    fn legend_leaving_by_legend_rule_sees_a_simultaneous_death() {
        ...
        let victim = game.create_object_from_definition(&vanilla("Doomed Bear", 2, 2), BOB, Zone::Battlefield);
        ...
        let kept = [first, second].into_iter().filter(|id| game.object(*id).is_some()).count();
        assert_eq!(kept, 1);
...
mod t1_delayed_trigger_incarnation {
    ...
    fn breach(blink: bool) -> Option<Zone> {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let bear = game.create_object_from_definition(&vanilla("Breach Bear", 2, 2), ALICE, Zone::Hand);
        let stable = game.object(bear).unwrap().stable_id;
        let breach = game.create_object_from_definition(&card("Through the Breach"), ALICE, Zone::Hand);
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Red, 5);
        let mut dm = Script { yes: true, ..Script::default() };
        ...
        if blink {
            ...
            game.move_object_with_etb_processing(exiled, Zone::Battlefield).unwrap();
        }
...
mod t2_state_trigger_retriggers {
    ...
    fn stifled_crocodile_trigger_triggers_again() {
        ...
        settle(&mut game, &mut queue, &mut dm);
        assert_eq!(game.stack.len(), 1, "no second instance while one is on the stack");
        let trigger = game.stack[0].ability_id.unwrap();
        cast(&mut game, &mut queue, BOB, stifle, &mut Script::targeting(vec![vec![Target::Object(trigger)]]));
...
mod t3_reflexive_trigger_uses_the_stack {
    ...
    fn manticore_reflexive_damage_waits_on_the_stack() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let fodder = game.create_object_from_definition(&vanilla("Big Fodder", 4, 4), ALICE, Zone::Battlefield);
        let manticore = game.create_object_from_definition(&card("Heart-Piercer Manticore"), ALICE, Zone::Hand);
        ...
        assert!(dm.target_prompts.is_empty(), "the ETB trigger has no targets");
        ...
        assert_eq!(game.stack.len(), 1, "the reflexive trigger is its own stack object");
        assert!(game.stack[0].is_ability);
        assert_eq!(life(&game, BOB), 20, "it has not resolved yet");
        assert_eq!(dm.target_prompts.len(), 1, "targets were chosen as it was put on the stack");
...
mod e2_6_explore_after_leaving {
    ...
    fn jadelight_killed_in_response_gets_no_counters_in_the_graveyard() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let ranger = game.create_object_from_definition(&card("Jadelight Ranger"), ALICE, Zone::Hand);
        let stable = game.object(ranger).unwrap().stable_id;
        game.player_mut(ALICE).unwrap().mana_pool.add(ManaSymbol::Green, 3);
        let mut dm = Script::default();
        ...
        let card_in_graveyard = game.move_object_by_effect(permanent, Zone::Graveyard).unwrap();
        resolve_all(&mut game, &mut queue, &mut dm);
        assert_eq!(game.object(card_in_graveyard).map(|o| o.zone), Some(Zone::Graveyard));
...
mod e2_7_initiative_venture_trigger {
    ...
    fn register_undercity() {
        let mut definition =
            ironsmith::cards::builders::CardDefinitionBuilder::new(CardId::new(), "Undercity")
                ...
                .build();
        for (room, leads_to) in [("Secret Entrance", vec!["Forge".to_string()]), ("Forge", Vec::new())] {
            definition.abilities.push(ironsmith::ability::Ability::triggered(
    ...
    fn taking_the_initiative_puts_a_venture_trigger_on_the_stack() {
        ...
        assert!(game.active_dungeon(ALICE).is_none(), "the venture has not happened yet");
        assert_eq!(game.stack.len(), 1, "the venture trigger");
        resolve_all(&mut game, &mut queue, &mut dm);
        assert_eq!(game.active_dungeon(ALICE).map(|d| d.dungeon_name.as_str()), Some("Undercity"));
    }
    ...
    fn initiative_passed_by_a_leaving_player_ventures() {
        ...
        let mut dm = Script::default();
        settle(&mut game, &mut queue, &mut dm);
        assert!(game.has_initiative(ALICE), "the active player takes the initiative");
        resolve_all(&mut game, &mut queue, &mut dm);
        assert_eq!(game.active_dungeon(ALICE).map(|d| d.dungeon_name.as_str()), Some("Undercity"));
    }
}
...
mod d1_09_per_event_trigger_matching {
    ...
    fn a_watcher_arriving_later_does_not_see_an_earlier_entry() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let warden = game.create_object_from_definition(&card("Soul Warden"), ALICE, Zone::Graveyard);
    ...
    fn a_watcher_leaving_later_still_sees_an_earlier_entry() {
        let mut game = new_game();
        let mut queue = TriggerQueue::new();
        let warden = game.create_object_from_definition(&card("Soul Warden"), ALICE, Zone::Battlefield);
        let spell = soul_warden_spell(
            &mut game,
    ...
    fn nested_scry_is_matched_before_the_next_nested_step() {
        ...
        let watcher = game.create_object_from_definition(
            &custom("Scry Watcher", vec![CardType::Creature], free(), "Whenever you scry, you gain 1 life."),
            ALICE,
            Zone::Graveyard,
        );
        let spell = soul_warden_spell(
        ...
        assert_eq!(life(&game, ALICE), 20, "the watcher wasn't there when Alice scried");
    }
}
