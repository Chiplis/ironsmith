...
fn resolve_chapter(game: &mut GameState, saga: ObjectId, index: usize, targets: Vec<ResolvedTarget>) {
...
fn creature(name: &str, color: ManaSymbol) -> CardDefinition {
    CardDefinitionBuilder::new(CardId::new(), name)
        ...
        .mana_cost(ManaCost::from_pips(vec![vec![ManaSymbol::Generic(3)], vec![color]]))
...
fn after_chapter_one() -> (GameState, [ObjectId; 3]) {
    ...
    let red = game.create_object_from_definition(&creature("Red Brute", ManaSymbol::Red), alice, Zone::Hand);
    let green = game.create_object_from_definition(&creature("Green Brute", ManaSymbol::Green), alice, Zone::Hand);
    let blue = game.create_object_from_definition(&creature("Blue Brute", ManaSymbol::Blue), alice, Zone::Hand);
...
fn chapter_one_lets_the_next_red_or_green_creature_be_cast_free_once() {
    ...
    assert!(cast_action(&game, blue).is_none(), "blue creatures are not covered");
    ...
    assert!(cast_action(&game, green).is_none(), "only the next such spell");
}

#[test]
fn casting_a_matching_spell_normally_uses_up_the_permission() {
    let (mut game, [red, green, _]) = after_chapter_one();
    game.player_mut(PlayerId::from_index(0)).unwrap().mana_pool.add(ManaSymbol::Red, 4);
    ...
    let paid = compute_legal_actions(&game, PlayerId::from_index(0))
        ...
        .expect("normal cast");
    cast(&mut game, paid);
    assert!(cast_action(&game, green).is_none(), "the next matching spell was already cast");
}
...
fn chapter_three_doubles_power_and_toughness_and_grants_trample() {
    ...
    game.turn.active_player = alice;
    let saga = game.create_object_from_definition(&definition(), alice, Zone::Battlefield);
    let bear = game.create_object_from_definition(&creature("Brute", ManaSymbol::Green), alice, Zone::Battlefield);
