...
fn exile_trigger_filters_owner_origin_and_counts_batches_once() {
    ...
    for owner in [alice, bob] {
        for zone in [Zone::Library, Zone::Graveyard, Zone::Hand, Zone::Battlefield] {
            for count in [0, 1, 3] {
                ...
                game.push_to_stack(ironsmith::game_state::StackEntry::ability(source, bob,
                    vec![ironsmith::Effect::exile_all(ironsmith::target::ObjectFilter::default().in_zone(zone).named("Exile probe"))]));
                ...
                let expected = usize::from(owner == alice && matches!(zone, Zone::Library | Zone::Graveyard) && count > 0);
                assert_eq!(game.stack.len(), expected, "owner={owner:?}, zone={zone:?}, count={count}");
                if expected != 0 { ironsmith::game_loop::resolve_stack_entry(&mut game).unwrap(); }
                let counters = game.object(source).unwrap().counters.get(&ironsmith::object::CounterType::PlusOnePlusOne).copied().unwrap_or(0);
                assert_eq!(counters, expected as u32, "owner={owner:?}, zone={zone:?}, count={count}");
            }
        }
    }
}
    ...
    for card_type in [CardType::Land, CardType::Sorcery] {
        for source_leaves in [false, true] {
            ...
            let probe = CardDefinitionBuilder::new(CardId::new(), "Top card probe")
                ...
                .mana_cost(ironsmith::mana::ManaCost::from_symbols(vec![ironsmith::mana::ManaSymbol::Generic(2)]))
                .build();
            ...
            ironsmith::combat_state::declare_attackers(&mut game, &mut combat,
                vec![(source, ironsmith::combat_state::AttackTarget::Player(bob))]).unwrap();
            let event = ironsmith::triggers::TriggerEvent::new_with_provenance(
                ironsmith::events::CreatureAttackedEvent::new(source, ironsmith::triggers::event::AttackEventTarget::Player(bob)),
                ironsmith::provenance::ProvNodeId::default());
            let mut queue = ironsmith::triggers::TriggerQueue::new();
            for entry in ironsmith::triggers::check_triggers(&game, &event) { queue.add(entry); }
            ironsmith::game_loop::put_triggers_on_stack(&mut game, &mut queue).unwrap();
            assert_eq!(game.stack.len(), 1);
            if source_leaves { game.move_object_by_effect(source, Zone::Graveyard); }
            ironsmith::game_loop::resolve_stack_entry(&mut game).unwrap();
            ironsmith::game_loop::put_triggers_on_stack(&mut game, &mut queue).unwrap();
            assert_eq!(game.stack.len(), usize::from(!source_leaves));
            if !source_leaves { ironsmith::game_loop::resolve_stack_entry(&mut game).unwrap(); }
            let exiled = game.find_object_by_stable_id(identity).unwrap();
            assert_eq!(game.object(exiled).unwrap().zone, Zone::Exile);
            game.player_mut(alice).unwrap().mana_pool.add(ironsmith::mana::ManaSymbol::Colorless, 5);
            let has_play = |game: &GameState, player| compute_legal_actions(game, player).iter().any(|a| match a {
            ...
            assert!(has_play(&game, alice), "attack grants play permission even if source left");
...
fn mixed_origins_share_one_trigger_only_when_exiled_simultaneously() {
    ...
    for separate_instructions in [false, true] {
        ...
        let filters = [Zone::Library, Zone::Graveyard].map(|zone|
            ironsmith::target::ObjectFilter::default().in_zone(zone).named("Mixed exile probe"));
        let effects = if separate_instructions {
            filters.into_iter().map(ironsmith::Effect::exile_all).collect()
        } else {
            let mut union = ironsmith::target::ObjectFilter::default();
            union.any_of = filters.to_vec();
            vec![ironsmith::Effect::exile_all(union)]
        };
        game.push_to_stack(ironsmith::game_state::StackEntry::ability(source, alice, effects));
        ironsmith::game_loop::resolve_stack_entry(&mut game).unwrap();
        let mut queue = ironsmith::triggers::TriggerQueue::new();
        ironsmith::game_loop::put_triggers_on_stack(&mut game, &mut queue).unwrap();
        assert_eq!(game.stack.len(), if separate_instructions { 2 } else { 1 },
...
fn exile_batch_preserves_each_triggers_duplicate_abilities_and_group_snapshots() {
    ...
    let alice = PlayerId::from_index(0);
    let mut game = GameState::new(vec!["Alice".into(), "Bob".into()], 20);
    let trigger = |count| ironsmith::triggers::Trigger::new(ZoneChangeTrigger::new()
        .from(Zone::Graveyard).to(Zone::Exile)
        .filter(ironsmith::target::ObjectFilter::default().nontoken()).count(count));
    let grouped = ironsmith::Ability::triggered(trigger(CountMode::OneOrMore),
        vec![ironsmith::Effect::gain_life(1)]);
    let each = ironsmith::Ability::triggered(trigger(CountMode::Each),
        vec![ironsmith::Effect::gain_life(2)]);
    let watcher = CardDefinitionBuilder::new(CardId::new(), "Batch watcher")
        .card_types(vec![CardType::Artifact])
        .with_ability(grouped.clone()).with_ability(grouped).with_ability(each).build();
    ...
    let probe = CardDefinitionBuilder::new(CardId::new(), "Batch object")
        .card_types(vec![CardType::Artifact]).build();
    for _ in 0..3 { game.create_object_from_definition(&probe, alice, Zone::Graveyard); }
    let mut dm = ironsmith::decision::SelectFirstDecisionMaker;
    ironsmith::effects::execute_effect(&mut game,
        &ironsmith::Effect::exile_all(ironsmith::target::ObjectFilter::default().in_zone(Zone::Graveyard)),
        &mut ironsmith::effects::EffectContext::new(source, alice, &mut dm)).unwrap();
    let mut queue = ironsmith::triggers::TriggerQueue::new();
    ironsmith::game_loop::drain_pending_trigger_events(&mut game, &mut queue);
    assert_eq!(queue.entries.len(), 5, "two identical grouped abilities plus three each-card triggers");
    let mut grouped_count = 0;
    for entry in &queue.entries {
        let matcher = entry.ability.trigger.downcast_ref::<ZoneChangeTrigger>().unwrap();
        if matcher.count_mode == CountMode::OneOrMore {
            grouped_count += 1;
            assert_eq!(entry.event_value_amount, Some(3));
            assert_eq!(entry.tagged_objects.get(&ironsmith::tag::TagKey::from(ironsmith_core::ZONE_CHANGE_GROUP_TAG)).expect("captured matched batch snapshots").len(), 3);
        }
    }
