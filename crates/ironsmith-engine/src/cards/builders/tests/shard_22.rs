...
pub(super) fn parse_oracle_grasping_current_keeps_named_multi_zone_search_surface() {
    ...
    let rendered = crate::runtime_display::compiled_text_lines(&def)
        .join(" ")
        .to_ascii_lowercase();
...
pub(super) fn esper_origins_strict_parser_and_compiled_text_regression() {
    ...
    let rendered = crate::runtime_display::compiled_text_lines(&front).join("\n");
    ...
    let back_rendered = crate::runtime_display::compiled_text_lines(&back).join("\n");
    assert!(
