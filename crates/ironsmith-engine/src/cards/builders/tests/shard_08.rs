...
pub(super) fn source_cost_compiled_text_does_not_leak_placeholder_surfaces() {
    ...
    let memory_jar_text = crate::runtime_display::compiled_text_lines(&memory_jar).join(" ");
    assert!(
    ...
    let spirit_guide_text = crate::runtime_display::compiled_text_lines(&spirit_guide).join(" ");
    assert!(
    ...
    let awakening_zone_text =
        crate::runtime_display::compiled_text_lines(&awakening_zone).join(" ");
    assert!(
    ...
    let fixed_counter_cost_text =
        crate::runtime_display::compiled_text_lines(&fixed_counter_cost).join(" ");
    assert!(
    ...
    let variable_counter_cost_text =
        crate::runtime_display::compiled_text_lines(&variable_counter_cost).join(" ");
    assert!(
...
pub(super) fn parse_dingus_egg_keeps_the_source_and_controller_linked() {
    ...
    let rendered = unprocessed_compiled_lines(&def)
        .join(" ")
        .to_ascii_lowercase();
    let canonical = crate::runtime_display::canonical_compiled_lines(&def).join(" ");
    assert!(
...
fn processor_activation_compiled_text(name: &str, text: &str) -> String {
    ...
    crate::runtime_display::unprocessed_compiled_lines(&def).join("\n")
}
