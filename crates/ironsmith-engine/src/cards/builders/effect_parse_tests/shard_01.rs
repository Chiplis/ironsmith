...
fn parse_rejects_divided_damage_distribution_clause() {
    ...
    let joined = crate::runtime_display::unprocessed_compiled_lines(&def)
        .join(" ")
        .to_ascii_lowercase();
    assert!(
            joined.contains(
...
fn parse_choose_land_of_each_basic_land_type_then_destroy() {
    ...
    let joined = crate::runtime_display::unprocessed_compiled_lines(&def).join(" ");
    assert!(
...
fn parse_noncreature_graveyard_from_battlefield_trigger_keeps_controller() {
    ...
    let yomiji_joined = crate::runtime_display::unprocessed_compiled_lines(&yomiji)
        .join(" ")
        .to_ascii_lowercase();
    assert!(
...
fn parse_supports_spent_to_cast_conditional_clause_chain() {
    ...
    let joined = crate::runtime_display::unprocessed_compiled_lines(&definition)
        .join(" ")
        .to_ascii_lowercase();
    assert!(
...
fn parse_labeled_trigger_line_as_triggered_ability() {
    ...
    let rendered = crate::runtime_display::unprocessed_compiled_lines(&def);
    assert!(
        rendered
            .iter()
