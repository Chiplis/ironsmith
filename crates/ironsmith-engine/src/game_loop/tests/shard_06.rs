...
impl DecisionMaker for NamedCastCostDecisionMaker {
    fn decide_mana_payment(
        ...
        use crate::mana_payment::{
            ManaPaymentResponse, ManaPaymentSourceKind, RequiredAlternativePayment,
        };
        if game
            .object(ctx.request.source)
            ...
            && let Some(name) = self.choices.pop_front()
        {
            ...
            let source = game
                ...
                .find(|id| {
                    game.object(*id)
                        .is_some_and(|card| card.name == name.as_str())
                })
                .unwrap();
            ...
            preferences
                ...
                .push(RequiredAlternativePayment {
                    source,
...
pub(super) fn native_escape_uses_exact_graveyard_choices_and_rolls_back_partial_payment() {
    ...
    assert!(
        i061_zone_contains_name(
            &game,
            &game.player(alice).unwrap().graveyard,
            "Native Escape Probe"
        ),
