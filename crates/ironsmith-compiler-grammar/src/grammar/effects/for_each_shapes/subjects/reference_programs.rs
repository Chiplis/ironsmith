...
pub(super) fn contains_effect_verb_outside_filter_zone(tokens: &[OwnedLexToken]) -> bool {
    let words = crate::lexer::parser_token_word_refs(tokens);
    let Some(found) = super::super::super::chain_splitting::find_chain_verb_words(&words) else {
        return false;
    };
    ...
    if found.kind == super::super::super::chain_splitting::ChainVerbKind::Exile
        && found.word_index > 0
        && words.get(found.word_index - 1) == Some(&"in")
    {
        return super::super::super::chain_splitting::find_chain_verb_words(
