...
pub(super) fn continuous_duration(input: &mut WordSliceInput<'_>) -> WResult<Until> {
    alt((simple_turn_duration, source_remains_on_battlefield, affected_object_tapped)).parse_next(input)
}
...
fn affected_object_tapped(input: &mut WordSliceInput<'_>) -> WResult<Until> {
    ...
    Ok(Until::ForAsLongAs(ironsmith_core::ContinuousDurationPredicate::ObjectTapped(
