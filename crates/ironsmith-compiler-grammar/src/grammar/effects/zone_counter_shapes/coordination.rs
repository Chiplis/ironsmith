...
pub fn parse_repeated_counter_placement_shape(
    ...
    loop {
        ...
        if (contains_counter_on_each(first_tokens) && contains_counter_on_each(second_tokens))
            || (has_static_counter_placement_head(first_tokens) && has_static_counter_placement_head(second_tokens))
        {
...
mod tests {
    ...
    fn repeated_counter_split_keeps_conjunctions_inside_the_first_filter() {
        ...
        assert_eq!(shape.first_tokens.iter().filter(|token| token.is_word("and")).count(), 1);
        assert!(shape.second_tokens.first().unwrap().is_word("a"));
        let shared_target = crate::lexer::lex_line("Put a +1/+1 counter and a shield counter on target creature.", 0).unwrap();
