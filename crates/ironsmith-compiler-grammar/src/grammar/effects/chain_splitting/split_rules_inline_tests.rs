...
fn enumerated_reanimation_target_count_is_not_an_action_boundary() {
    ...
    for count in ["one, two, or three", "one, two or three", "one or two"] {
        let source = format!(
            "Put {count} target creature cards from graveyards onto the battlefield under your control."
        );
        let tokens = crate::lexer::lex_line(&source, 0).unwrap();
        let parts =
            crate::grammar::effects::chain_splitting::split_segments_on_comma_effect_head_tokens(
                vec![&tokens],
            );
    ...
    for tail in [", then draw a card", ", draw a card", " or draw a card"] {
        let source = format!(
            "Put one, two, or three target creature cards from graveyards onto the battlefield under your control{tail}."
        );
        let tokens = crate::lexer::lex_line(&source, 0).unwrap();
