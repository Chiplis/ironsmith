...
pub fn parse_for_each_count_value_words(words: &[&str]) -> Option<(Value, usize)> {
    ...
    // Exact event-relative cardinalities must win before the generic
    // `... this way` object-memory route. In particular, cards looked at
    // while scrying are the trigger's event amount; they are not a tagged
    // current card set.
