...
pub fn parse_source_counter_condition(
    ...
    let pronoun = crate::word_primitives::matching_value(
        &tail_words,
        &[
            (
                &["on", "him"][..],
                ironsmith_core::SourceCounterPronounSurface::Him,
            ),
            (
                &["on", "her"][..],
                ironsmith_core::SourceCounterPronounSurface::Her,
            ),
        ],
    );
...
fn is_source_condition_subject(tokens: &[OwnedLexToken]) -> bool {
    ...
    crate::util::is_source_reference_words(&view.word_refs())
}
...
#[cfg(test)]
#[path = "condition_shapes_inline_tests.rs"]
mod tests;
