...
#[test]
fn splits_keyword_before_quoted_activated_ability() {
    let tokens = lex("vigilance and \"{T}: Add one mana of any color.\"");
    let segments = split_trailing_grant_segments(&tokens);
    assert_eq!(segments.len(), 2, "{segments:#?}");
    assert_eq!(render_token_slice(&segments[0]), "vigilance");
    assert_eq!(
        render_token_slice(&segments[1]),
