...
fn typed_attached_restriction_shapes_preserve_static_semantics() {
    ...
    let tokens = crate::lexer::lex_line("Equipped creature can't be blocked.", 0).unwrap();
    let abilities = parse_static_ability_ast_line_lexed(&tokens)
        .expect("parse equipped-creature evasion")
        .expect("equipped-creature evasion should be a static ability");
    assert!(
        matches!(
            abilities.as_slice(),
