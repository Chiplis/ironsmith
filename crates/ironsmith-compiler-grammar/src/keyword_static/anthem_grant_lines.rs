...
fn defending_player_controlled_card_types_from_condition_tokens(
    ...
    let condition = crate::grammar::conditions::parse_control_condition(
        condition_tokens,
...
fn filtered_object_animation_abilities(
    filter: ObjectFilter,
    shape: crate::grammar::effects::become_shapes::FilteredObjectAnimationShape<'_>,
) -> Vec<StaticAbility> {
...
fn split_union_grant_subjects(
    ...
    for (idx, token) in subject_tokens.iter().enumerate() {
        ...
        let left_subject = if crate::word_primitives::parse_sequence_complete(&left_words, &["you"])
        {
...
pub fn parse_granted_keyword_static_line(
    ...
    fn parse_granted_alternative_cast_static(
        ...
        let spec = match anthem_grant_grammar::parse_granted_alternative_cast_keyword_tokens(
            ...
            Some(anthem_grant_grammar::GrantedAlternativeCastKeyword::Miracle) => {
                ...
                extract_grant_spec_from_subject(
                    subject_tokens,
                    crate::model::CompilerGrantableCore::miracle_from_cards_mana_cost_reduced_by(
                        reduction,
                    ),
                )?
            }
    ...
    if verb_facts.starts_with_as_long_as
        && !verb_facts.tail_has_have
        && verb_facts.tail_has_get_or_be
    {
        return Ok(None);
    }
    ...
    let tail_sentences = crate::grammar::primitives::split_lexed_slices_on_period(&tail_tokens);
...
pub fn parse_subject_loses_keywords_line(
    ...
    // A complete attached-object "has ... and loses ..." clause is not a
    // subject-loss clause with incidental prefix text.
    if attached_grammar::parse_attached_has_and_loses_tokens(tokens).is_some() {
        return Ok(None);
    }
...
pub fn parse_lose_all_abilities_and_base_pt_line(
    ...
    if shape.becomes {
        ...
        let Some(power_toughness) =
            crate::grammar::effects::become_shapes::parse_become_base_pt_words(
        ...
        let Some(descriptor) =
            crate::grammar::effects::become_shapes::parse_become_creature_descriptor_words(
    ...
    if let Some(modifier_word) = shape.base_power_toughness_word
        && let Some(modifier_token) = words.get(modifier_word)
    ...
    best.map(|(_, start, filter)| {
        if start > 0 {
            crate::parse_loss::record(
                ...
                format!(
                    "parsed '{}' as suffix of '{}'",
                    crate::lexer::token_word_refs(&tokens[start..]).join(" "),
                    crate::lexer::token_word_refs(tokens).join(" ")
                ),
            );
        }
...
pub fn parse_static_condition_clause(
    ...
    // resulting filter to the affected target instead of reducing the final
    // object noun to `TargetMatches`.
    if matches!(
        clause_words.get(..3),
        Some(["its", "controller", "controls"])
    ) {
        let mut explicit_controller = vec![
            OwnedLexToken::word("you".to_string(), TextSpan::synthetic()),
            OwnedLexToken::word("control".to_string(), TextSpan::synthetic()),
        ];
        explicit_controller.extend_from_slice(&tokens[3..]);
        if let Some(mut control) = crate::grammar::conditions::parse_control_condition(
            &explicit_controller,
            crate::grammar::conditions::ControlConditionOptions {
                allow_that_player: false,
                allow_opponent_players: false,
    ...
    let count_condition_tokens =
        crate::grammar::leaf::parse_leaf_static_condition_intro_prefix_tokens(&tokens)
            .map(|parsed| parsed.rest)
            .unwrap_or(&tokens);

    if let Some(mut control_condition) = crate::grammar::conditions::parse_control_condition(
        ...
        // whose affected objects are currently being evaluated. Preserve the
        // quantitative control condition before the general predicate
        // fallback can reduce it to an existential PlayerControls test.
        if control_condition.player == crate::cards::builders::PlayerAst::That
            && control_condition.filter.controller.is_none()
        {
    ...
    if let Some(ownership_condition) = crate::grammar::conditions::parse_ownership_condition(
        count_condition_tokens,
        crate::grammar::conditions::OwnershipConditionOptions {
            allow_opponent_players: true,
    ...
    // Static conditions share the same predicate language as intervening-if
    // and effect conditions.  Keep the specialized surface-preserving cases
...
fn anthem_turn_threshold_player_filter(
    player: crate::grammar::anthem_grants::TurnThresholdPlayer,
) -> PlayerFilter {
    use crate::grammar::anthem_grants::TurnThresholdPlayer;
    match player {
...
fn explicit_source_counter_surface(
    ...
    crate::util::source_reference_surface_for_words(source_words).or_else(|| {
        (source_words.len() > 1)
            .then(|| crate::util::this_source_surface_for_words(source_words))
            .flatten()
    })
}
...
pub fn parse_anthem_clause(
    ...
    let modifier_words =
        crate::lexer::token_word_refs(tokens.get(get_idx + 1..tail_end).unwrap_or_default());
...
fn parse_where_x_y_bindings(tokens: &[OwnedLexToken]) -> Option<(Value, Value)> {
    ...
    let x_words = crate::lexer::token_word_refs(shape.x_tokens);
    let y_words = crate::lexer::token_word_refs(shape.y_tokens);
...
pub fn parse_type_color_addition_clause(
    ...
    if shape.scopes.is_empty() {
        let descriptor_word_storage = crate::lexer::token_word_refs(shape.descriptor_tokens);
    ...
    for scope in shape.scopes {
        match scope {
            ...
            anthem_grant_grammar::TypeColorScope::Types { qualifier_tokens } => {
                let qualifiers = crate::lexer::token_word_refs(qualifier_tokens);
                if qualifiers
                    .iter()
...
pub fn parse_soulbond_shared_line(
    ...
    let subject_words = crate::lexer::token_word_refs(shape.subject_tokens);
...
mod dynamic_anthem_tests {
    ...
    fn winning_keyword_grant_route_moves_trailing_mana_source_predicate_into_spell_filter() {
        fn grant(ability: &StaticAbilityAst) -> Option<(&ObjectFilter, bool)> {
            match ability {
    ...
    #[test]
    fn source_anthem_keeps_one_opponents_graveyard_threshold() {
        let text = "This creature gets +4/+4 as long as an opponent has ten or more cards in their graveyard.";
        ...
        let tokens = lex_line(text, 0).expect("conditional source anthem should lex");
        let routed = super::parse_static_ability_ast_line_lexed(&tokens)
            .expect("static-line registry should not reject the anthem")
            .expect("static-line registry should claim the anthem");
