...
fn try_parse_simple_beginning_of_combat_trigger_lexed(
    ...
    let phase_end = if crate::word_primitives::parse_sequence_prefix(
        &words,
        &["the", "beginning", "of", "combat"],
    ) {
        4
    } else if crate::word_primitives::parse_sequence_prefix(&words, &["beginning", "of", "combat"])
    {
        3
    ...
    let suffix = &words[phase_end..];
    if !suffix.is_empty() && suffix.first() != Some(&"on") {
        return None;
    }
    Some(TriggerSpec::BeginningOfCombat(
        parse_possessive_clause_player_filter(&words),
    ))
}

fn try_parse_simple_source_enters_trigger_lexed(
    raw_tokens: &[OwnedLexToken],
) -> Option<TriggerSpec> {
    let tokens = trim_edge_punctuation_tokens(strip_leading_trigger_intro(raw_tokens));
    let words = crate::lexer::token_word_refs(tokens);
    if words.len() != 3 || words.first() != Some(&"this") || words.last() != Some(&"enters") {
        return None;
    }
    let surface = this_source_surface_for_words(&words[..2])?;
    Some(apply_leading_trigger_intro_surface(
        this_enters_battlefield_trigger_spec(
            Some(surface),
            ironsmith_core::trigger_model::TriggerSubjectNumber::Singular,
            None,
        ),
