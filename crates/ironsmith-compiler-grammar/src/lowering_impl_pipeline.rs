...
mod tests {
    ...
    #[test]
    fn delayed_exile_all_cardinality_survives_the_semantic_pipeline() -> Result<(), CardTextError> {
        let builder = CardDefinitionBuilder::new(CardId::new(), "Delayed Exile All")
            .card_types(vec![CardType::Creature]);
        let text = "When this creature leaves the battlefield, exile all tokens created with it at the beginning of the next end step.";
        let mut context = crate::parse_context_for_builder(&builder, text, false);
        ...
        assert!(
            parsed_debug.contains("ExileAll"),
        ...
        assert!(
            ...
            "prepared semantic AST lost exhaustive exile cardinality:\n{prepared_debug}"
        );
        Ok(())
    }
    ...
    fn line_semantic_facts_survive_parsed_and_normalized_stages() -> Result<(), CardTextError> {
        ...
        assert_eq!(
            ...
            Some(PredicateAst::And(
                Box::new(PredicateAst::YourTurn),
                Box::new(PredicateAst::DoThisMaxTimesEachTurn(1)),
            ))
        );
        Ok(())
    }
