...
fn parse_inline_look_exile_face_down_permission_bundle(
    ...
    let has_face_down_exile = exile_words.iter().any(|word| *word == "exile")
        && exile_words
            .windows(2)
...
fn parse_optional_result_exile_choice_play_bundle(
    ...
    let lexical_owner = optional_words.iter().any(|word| *word == "may")
        ...
        && permission_words.iter().any(|word| *word == "may")
        && permission_words
            .iter()
    ...
    match target {
        TargetAst::PlayerOrPlaneswalker(PlayerFilter::TargetPlayerOrControllerOfTarget, span) => {
            *target = match iteration {
                ...
                MixedTargetIteration::Object => TargetAst::Tagged(TagKey::from(IT_TAG), *span),
            };
        }
        TargetAst::WithCount(inner, _) | TargetAst::WithCountValue(inner, ..) => {
...
#[cfg(test)]
#[path = "bundle_rules_inline_tests.rs"]
mod tests;

#[path = "bundle_rules/bundle_rules_core_programs.rs"]
mod bundle_rules_core_programs;
pub use bundle_rules_core_programs::parse_typed_effect_bundle_lexed;
#[path = "bundle_rules/bundle_rules_reference_programs.rs"]
...
#[path = "bundle_rules/bundle_rules_object_action_programs.rs"]
...
#[path = "bundle_rules/bundle_rules_resource_programs.rs"]
