...
use crate::cards::builders::{
    CardDefinitionBuilder, CardTextError, GiftTimingAst, KeywordAction, ParseAnnotations,
...
pub(super) fn apply_line_ast(
    ...
    match chunk {
        ...
        NormalizedLineChunk::OptionalCostWithCastTrigger { cost, prepared } => {
...
fn has_unique_cross_line_self_replacement_target_condition(
    ...
    prepared
        ...
        .iter()
        .map(|annotated| {
            count_nested_anaphoric_target_conditions(std::slice::from_ref(&annotated.effect))
        })
        .sum::<usize>()
        == 1
}
