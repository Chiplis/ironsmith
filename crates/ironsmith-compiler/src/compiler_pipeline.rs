...
pub fn parse_text_to_semantic_document_with_context(
    ...
    document_parser::parse_text_to_semantic_document_with_context(context, card, text)
}
...
pub fn normalize_parsed_document(
    ast: ParsedCardAst,
    seed: CardDefinitionBuilder,
) -> Result<NormalizedCardAst, CardTextError> {
    lower::normalize_parsed_card_ast_for_lowering(ast, seed)
}
...
pub fn parse_text_with_annotations_lowered(
    ...
    let mut context =
        crate::parse_context_for_builder(&builder.card_builder, &text, allow_unsupported);
    let lowered =
...
fn parse_text_to_semantic_document(
    ...
    let mut context =
        crate::parse_context_for_builder(&builder.card_builder, &text, allow_unsupported);
    parse_text_to_semantic_document_with_context(&mut context, builder.split_face().0, text)
}
...
pub fn parse_text_with_annotations_lowered_with_facts_context(
    ...
    let (card, seed) = builder.split_face();
    let parsed = crate::canonical_pipeline::parse_card_ast_with_context(context, card, text)?;
    crate::lowering::lower_card_ast(&mut RuntimeCardAstMaterializer { seed: Some(seed) }, parsed)
}

/// Lowers a recognized card AST, starting from the definition seed its caller
/// handed in. The seed is taken on the single `materialize` call this
/// materializer ever receives.
struct RuntimeCardAstMaterializer {
    seed: Option<CardDefinitionBuilder>,
}
...
impl CardAstMaterializer for RuntimeCardAstMaterializer {
    ...
    fn materialize(&mut self, ast: ParsedCardAst) -> Result<Self::RuntimeDocument, Self::Error> {
        let prepared = {
            ...
            let seed = self.seed.take().ok_or_else(|| {
                CardTextError::ParseError(
                    "card AST materializer used twice: the definition seed is already spent"
                        .to_string(),
                )
            })?;
            normalize_parsed_document(ast, seed)?
        };
...
mod tests {
    ...
    fn delayed_exile_all_cardinality_survives_the_semantic_pipeline() -> Result<(), CardTextError> {
        ...
        let mut context = crate::parse_context_for_builder(&builder.card_builder, text, false);
        ...
        let prepared = normalize_parsed_document(parsed, CardDefinitionBuilder::seed())?;
    ...
    fn time_vault_turn_skip_followup_survives_preparation() -> Result<(), CardTextError> {
        ...
        let (semantic, _) = parse_text_to_semantic_document(builder, text.to_string(), false)?;
        let parsed = parse_semantic_document(semantic)?;
        normalize_parsed_document(parsed, CardDefinitionBuilder::seed())?;
        Ok(())
    }
