...
pub(crate) fn starts_with<T: PartialEq>(items: &[T], prefix: &[T]) -> bool {
    items.len() >= prefix.len() && items[..prefix.len()] == *prefix
}

pub(crate) fn ends_with<T: PartialEq>(items: &[T], suffix: &[T]) -> bool {
    items.len() >= suffix.len() && items[items.len() - suffix.len()..] == *suffix
}

pub(crate) fn contains<T: PartialEq>(items: &[T], expected: &T) -> bool {
    items.iter().any(|item| item == expected)
}

pub(crate) fn contains_any<T: PartialEq>(items: &[T], expected: &[T]) -> bool {
    expected.iter().any(|candidate| contains(items, candidate))
}

pub(crate) fn contains_all<T: PartialEq>(items: &[T], expected: &[T]) -> bool {
...
pub(crate) fn starts_with_any<T: PartialEq>(items: &[T], patterns: &[&[T]]) -> bool {
    patterns.iter().any(|pattern| starts_with(items, pattern))
}

pub(crate) fn ends_with_any<T: PartialEq>(items: &[T], patterns: &[&[T]]) -> bool {
...
pub(crate) fn strip_prefix<'a, T: PartialEq>(items: &'a [T], prefix: &[T]) -> Option<&'a [T]> {
    starts_with(items, prefix).then(|| &items[prefix.len()..])
}

pub(crate) fn strip_suffix<'a, T: PartialEq>(items: &'a [T], suffix: &[T]) -> Option<&'a [T]> {
    ends_with(items, suffix).then(|| &items[..items.len() - suffix.len()])
}

pub(crate) fn strip_any_prefix<'a, 'p, T: PartialEq>(
    items: &'a [T],
    patterns: &'p [&'p [T]],
) -> Option<(&'p [T], &'a [T])> {
    patterns
...
pub(crate) fn strip_any_suffix<'a, 'p, T: PartialEq>(
    items: &'a [T],
    patterns: &'p [&'p [T]],
) -> Option<(&'p [T], &'a [T])> {
    patterns
