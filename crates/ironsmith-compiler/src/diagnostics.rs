...
mod tests {
    ...
    fn parse_annotations_records_each_kind_of_note() {
        ...
        assert_eq!(annotations.normalized_lines[&1], "draw a card");
        assert_eq!(annotations.original_lines[&1], "Draw a card.");
        assert_eq!(annotations.normalized_char_maps[&1], vec![0, 1, 2]);
    }

    #[test]
    fn synthetic_span_is_zeroed() {
        assert_eq!(
            TextSpan::synthetic(),
            TextSpan {
                line: 0,
