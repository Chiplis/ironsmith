...
use crate::activation_and_restrictions::last_created_token_info;
...
fn source_tokens_prove_repeatable_instant_timing_until_end_of_turn(
    ...
    let words: Vec<String> = crate::lexer::token_word_refs(tokens)
        .into_iter()
...
fn fuse_repeatable_mana_payment_prevention_until_end_of_turn(
    ...
    source_tokens: &[crate::lexer::OwnedLexToken],
) -> bool {
...
fn clash_win_optional_top_replacement_program(
    ...
    let (clash_id, clash_effect, return_effect, followup) = match compiled {
        ...
        [grouped_effect, followup] => {
            ...
            clash_effect.downcast_ref::<crate::effects::ClashEffect>()?;
...
pub(super) fn rewrite_apply_line_ast(
    ...
    if let Some(program) = builder.spell_effect.as_mut() {
        let authored_tokens = crate::lexer::lex_line(&info.raw_line, info.line_index)
...
fn bind_authored_chosen_creature_static_filters(
    ...
    let words = crate::lexer::parser_token_word_refs(source_tokens);
    if !words
        .windows(3)
...
fn source_line_ability_word_label(info: &LineInfo) -> Option<String> {
    let split =
        crate::grammar::document_shapes::parse_statement_label_split_tokens(&info.source_tokens)?;
    if crate::grammar::document_shapes::parse_preserved_keyword_label_tokens(split.label_tokens)
        .is_some()
    {
        return None;
    }
    let label = crate::lexer::render_token_slice(split.label_tokens)
        .trim()
        .to_string();
...
fn is_source_line_first_spell_cost_reduction_and_flash_group(
    ...
    let crate::static_abilities::StaticAbilityPayload::Grants(flash) = &flash_ability.payload
    else {
        return false;
    };
...
fn bind_fixed_reduction_to_as_long_as_draw_threshold(
    ...
    if let Ok(Some(reparsed)) =
        crate::keyword_static::parse_spells_cost_modifier_line(source_tokens)
    ...
    let Some(parsed_cost) = crate::grammar::leaf::parse_leaf_fixed_mana_cost_prefix_tokens(
    ...
    let Some(condition_token) = crate::grammar::static_keyword_shapes::parse_word_token_offset(
        source_tokens,
        as_long_as_word + 3,
    ) else {
        return;
    ...
    let lexical_threshold = words
        ...
        .and_then(|word| crate::util::parse_number_word_i32(word))
    ...
    let condition = parsed_condition
        ...
        .unwrap_or(crate::effect::Condition::ValueComparison {
            left: crate::effect::Value::MaxCardsDrawnThisTurn(PlayerFilter::You),
            operator: ironsmith_core::ValueComparisonOperator::GreaterThanOrEqual,
            right: crate::effect::Value::Fixed(threshold),
        });
    let display = crate::lexer::render_token_slice(condition_tokens)
        .trim()
        .trim_end_matches('.')
        .to_string();
...
fn lower_parsed_ability_chunk(
    ...
    if semantic_facts.triggered_ability.compiler_ability.is_some() {
        if let Some(split) =
            crate::grammar::semantic_lowering::parse_comma_split_tokens(&authored_tokens)
            && let Some(serial_modifiers) =
                crate::effect_sentences::parse_serial_target_pt_modifiers_sentence(split.after)?
        {
            // The line-level semantic fact is created before the later
            // authored-surface reconciliation. Rebuild this exact serial
            // target list from the intact line so the stale prepared payload
            // cannot merge its third independent target into an ordinary
            // object set.
            parsed_ability.parsed.effects_ast = Some(serial_modifiers);
            parsed_ability.prepared = None;
        }
        let parsed_ability = super::rewrite_lower_prepared_ability(parsed_ability)?;
        if let Some(effects_ast) = parsed_ability.effects_ast.as_deref() {
    ...
    let source_tail =
        crate::grammar::semantic_lowering::parse_comma_split_tokens(&info.source_tokens)
            .map(|split| split.after);
    let reconciled_dynamic =
        crate::semantic_line_parsing::
            dynamic_zone_change_group_token_creation_from_authored_trigger(&authored_tokens)?
            .or(crate::semantic_line_parsing::
                dynamic_zone_change_group_token_creation_from_authored_trigger(
                    &info.source_tokens,
                )?);
    let reconciled_looked_hand = authored_tail
        .as_ref()
        .and_then(|tail| crate::semantic_line_parsing::exact_looked_hand_optional_cast_bundle(tail))
        .or_else(|| {
            source_tail.as_ref().and_then(|tail| {
                crate::semantic_line_parsing::exact_looked_hand_optional_cast_bundle(tail)
            })
        })
        .or_else(|| {
            crate::semantic_line_parsing::exact_looked_hand_optional_cast_bundle(&authored_tokens)
        })
        .or_else(|| {
            crate::semantic_line_parsing::exact_looked_hand_optional_cast_bundle(
                &info.source_tokens,
            )
        });
    let reconciled_targeted_same_name_cast = authored_tail
        .as_ref()
        .and_then(|tail| {
            crate::semantic_line_parsing::exact_target_same_name_graveyard_may_cast_bundle(tail)
        })
        .or_else(|| {
            source_tail.as_ref().and_then(|tail| {
                crate::semantic_line_parsing::exact_target_same_name_graveyard_may_cast_bundle(tail)
    ...
    let reconciled_graveyard_copy_cast = authored_tail
        .as_ref()
        .and_then(|tail| {
            crate::semantic_line_parsing::exact_graveyard_card_copy_cast_sequence(tail)
        })
        .or_else(|| {
            source_tail.as_ref().and_then(|tail| {
                crate::semantic_line_parsing::exact_graveyard_card_copy_cast_sequence(tail)
    ...
    let reconciled_quantified_token_rules = authored_tail
        .as_ref()
        .map(|tail| {
            crate::effect_sentences::parse_quantified_token_creation_with_embedded_rules(tail)
        })
        .transpose()?
        .flatten()
        .or(source_tail
    ...
    let reconciled_attacking_opponents_pump = if let Some(tail) = authored_tail.as_ref() {
        ...
        if words
            ...
            && words.contains(&"opponents")
            && words.contains(&"planeswalkers")
            && words.windows(2).any(|window| window == ["they", "control"])
        {
            Some(crate::effect_sentences::parse_effect_sentences_lexed(tail)?)
        } else {
            None
        }
    } else {
    ...
    bind_graveyard_cast_trigger_to_triggering_permanent_entry(&mut ability, &authored_tokens);
    ...
    if let Some(program) = program {
        ...
        bind_selected_exile_card_play_permission(program);
        bind_chosen_opponent_followup_actions(program);
        bind_ordered_opponent_discard_draw_and_search_actor(program);
        bind_reciprocal_library_inspection_actors(program);
...
fn bind_authored_source_and_each_opponent_creature_exile(
    program: &mut crate::resolution::ResolutionProgram,
    authored_tokens: &[crate::lexer::OwnedLexToken],
) -> bool {
    ...
    let Some(source_surface) =
        crate::util::source_reference_surface_for_words(&words[exile_index + 1..tail_index])
    else {
        return false;
    };
...
fn preserve_separate_copy_instruction_surface(
    ...
    let authored_sentences = crate::lexer::split_lexed_sentences(source_tokens);
    let has_permanent_copy_reminder = authored_sentences.iter().any(|sentence| {
        crate::lexer::parser_token_word_refs(sentence).as_slice()
            == [
                "a",
                "copy",
                "of",
...
fn reconcile_exactly_one_creature_intervening_condition(
    ...
    let words = crate::lexer::token_word_refs(source_tokens);
    if !words
        .windows(6)
...
fn bind_equipped_attack_draw_reveal_result(
    ...
    let source_words = crate::lexer::parser_token_word_refs(source_tokens);
...
fn normalize_graveyard_card_copy_cast_program(program: &mut crate::resolution::ResolutionProgram) {
    ...
    fn tagged_graveyard_exile_tag(effect: &crate::effect::Effect) -> Option<crate::TagKey> {
        ...
        if !crate::util::is_sentence_helper_tag(tagged.tag.as_str(), "exiled") {
            return None;
        }
...
fn bind_dynamic_power_owner_exile_permission(
    ...
    let authored_tokens = crate::lexer::lex_line(raw_line, 0).ok();
    let words =
        crate::lexer::parser_token_word_refs(authored_tokens.as_deref().unwrap_or(source_tokens));
    const EXILE: &[&str] = &[
...
fn bind_each_opponent_explicit_you_token_unless_iterated_sacrifice(
    program: &mut crate::resolution::ResolutionProgram,
    source_tokens: Option<&[crate::lexer::OwnedLexToken]>,
) {
...
mod quantified_unless_actor_binding_tests {
    ...
    fn explicit_you_token_and_that_opponent_sacrifice_recover_distinct_actors() {
        ...
        let authored = crate::lexer::lex_line(
...
mod dynamic_power_owner_exile_permission_tests {
    ...
    fn exact_authored_bundle_reconciles_to_dynamic_owner_linked_permission() {
        ...
        let lossy_tokens = crate::lexer::lex_line(
...
fn reconcile_authored_source_exiled_return_runtime(
    ...
    let Some(surface) = crate::effect_sentences::parse_exiled_with_source_move_surface(
...
fn source_words_contain(tokens: &[crate::lexer::OwnedLexToken], phrase: &[&str]) -> bool {
...
fn bind_then_if_source_untap_and_transform(
    ...
    if !words
        ...
        || !words.contains(&"untap")
        || !words.contains(&"transform")
    {
        return;
    }
...
fn authored_compound_damage_fanout_effects(
    ...
    if crate::lexer::split_lexed_sentences(&info.source_tokens).len() != 1 {
        return Ok(None);
    }
    let normalized_tokens = crate::lexer::lex_line(&info.normalized.normalized, info.line_index)?;
    let raw_verb = info
        .source_tokens
        .iter()
...
fn preserve_authored_unenchanted_destroy_filter(
    ...
    if !says_unenchanted || !words.contains(&"destroy") || !words.contains(&"regenerated") {
        return false;
    }
    ...
    fn mark(effect: &mut EffectAst, aura: &ObjectFilter) -> bool {
        match effect {
            EffectAst::SubjectVerb(SubjectVerbEffectAst {
                ...
                if let Some(filter) = crate::effect_sentences::target_object_filter_mut(target)
...
fn lower_statement_chunk(
    ...
    let authored_tokens = crate::lexer::lex_line(&info.raw_line, info.line_index)
        .unwrap_or_else(|_| info.source_tokens.clone());
    let authored_sentences = crate::lexer::split_lexed_sentences(&authored_tokens);
    if let [choose, reciprocal_damage] = authored_sentences.as_slice()
        && crate::lexer::parser_token_word_refs(choose).as_slice()
            == [
                "choose", "target", "creature", "you", "control", "and", "target", "creature",
                "an", "opponent", "controls",
            ]
        && crate::lexer::parser_token_word_refs(reciprocal_damage).as_slice()
            == [
                "each",
                "of",
                "those",
            ...
            ]
    {
        let Some(mut authored_reciprocal) =
            crate::effect_sentences::parse_choose_target_prelude_sentence(choose)?
        else {
            return Err(CardTextError::InvariantViolation(
                "authored reciprocal target declaration stopped parsing".to_string(),
            ));
        };
        authored_reciprocal.extend(crate::effect_sentences::parse_effect_sentences_lexed(
            reciprocal_damage,
        )?);
    ...
    if authored_sentences.len() == 4 {
        ...
        if let Some(authored_tempting_offer) =
            crate::effect_sentences::parse_tempting_offer_copy_spell_sequence(&sentence_inputs, 0)?
        {
    ...
    if authored_sentences.len() == 3
        ...
        && let Ok(authored_replacement) =
            crate::effect_sentences::parse_effect_sentences_lexed(&authored_tokens)
        && matches!(
    ...
    let authored_words = crate::lexer::parser_token_word_refs(&authored_tokens);
    ...
    {
        let mut authored_destroy = crate::effect_sentences::parse_destroy(&authored_tokens[1..])?;
    ...
    if crate::effect_sentences::attach_inline_token_granted_abilities_to_last_create(
    ...
    if crate::effect_sentences::damage_clause_has_terminal_unpreventable_rider(&info.source_tokens)
    {
        ...
        for effect in &mut prepared.effects {
            if let EffectAst::SelfReplacement { if_true, .. } = effect {
                for replacement_effect in if_true {
                    crate::effect_sentences::mark_damage_ast_unpreventable(replacement_effect);
                }
        ...
        if !marked_replacement {
            for effect in &mut prepared.effects {
                crate::effect_sentences::mark_damage_ast_unpreventable(effect);
            }
        }
    }
...
/// immediately following permission for "that card". The executable
/// distinction matters: the permission must cover only the selected card,
/// not every card exiled by the preceding dynamic-count effect.
pub(super) fn bind_selected_exile_card_play_permission(
    program: &mut crate::resolution::ResolutionProgram,
) {
...
/// Preserve the shared opponent in an ordered discard/draw result clause and
/// the controller of a following search-then-discard procedure.
fn bind_ordered_opponent_discard_draw_and_search_actor(
    program: &mut crate::resolution::ResolutionProgram,
) {
    fn rewrite_sequence(
        sequence: &crate::effects::SequenceEffect,
    ) -> crate::effects::SequenceEffect {
        let mut sequence = sequence.clone();
        sequence.effects = sequence
            .effects
            .iter()
            .map(|effect| {
                ...
                {
                    let mut conditional = conditional.clone();
                    conditional.if_true = conditional.if_true.iter().map(rewrite_effect).collect();
                    conditional.if_false =
                        conditional.if_false.iter().map(rewrite_effect).collect();
                    crate::effect::Effect::new(conditional)
                } else {
                    rewrite_effect(effect)
                }
            })
            .collect();
        ...
        if sequence.surface == ironsmith_core::SequenceSurface::Coordinated
            ...
            && let Some(with_id) =
                sequence.effects[0].downcast_ref::<crate::effects::WithIdEffect>()
            && let Some(discard) = with_id
                .effect
                .downcast_ref::<crate::effects::DiscardEffect>()
            && let Some(draw) =
                sequence.effects[1].downcast_ref::<crate::effects::DrawCardsEffect>()
            && discard.player == PlayerFilter::Opponent
            && draw.player == PlayerFilter::Opponent
            && matches!(draw.count.unhinted(), Value::EffectMetric { effect_id, .. } if *effect_id == with_id.id)
        {
            let tag = crate::tag::TagKey::from("chosen_discarding_opponent");
            let chosen = PlayerFilter::TaggedPlayer(tag.clone());
            let mut rebound_discard = discard.clone();
            rebound_discard.player = chosen.clone();
            let mut rebound_with_id = with_id.clone();
            rebound_with_id.effect = Box::new(crate::effect::Effect::new(rebound_discard));
            let mut rebound_draw = draw.clone();
            rebound_draw.player = chosen;
            sequence.effects = vec![
                crate::effect::Effect::new(crate::effects::ChoosePlayerEffect::new(
                    PlayerFilter::You,
                    PlayerFilter::Opponent,
                    tag,
                )),
                crate::effect::Effect::new(rebound_with_id),
                crate::effect::Effect::new(rebound_draw),
            ];
        }

        if sequence.surface == ironsmith_core::SequenceSurface::CommaThen
            && sequence.effects.len() == 2
            && let Some(search_sequence) =
                sequence.effects[0].downcast_ref::<crate::effects::SequenceEffect>()
            && let Some(discard) =
                sequence.effects[1].downcast_ref::<crate::effects::DiscardEffect>()
            && discard.random
            && let Some(search) = search_sequence.effects.iter().find_map(|effect| {
                effect
                    .downcast_ref::<crate::effects::ChooseObjectsEffect>()
                    .filter(|choice| choice.is_search)
            })
            && search_sequence.effects.iter().any(|effect| {
                effect
                    .downcast_ref::<crate::effects::ShuffleLibraryEffect>()
                    .is_some_and(|shuffle| shuffle.player == search.chooser)
            })
            && discard.player != search.chooser
        {
            let mut discard = discard.clone();
            discard.player = search.chooser.clone();
            sequence.effects[1] = crate::effect::Effect::new(discard);
        }
        sequence
    }

    fn rewrite_effect(effect: &crate::effect::Effect) -> crate::effect::Effect {
        if let Some(sequence) = effect.downcast_ref::<crate::effects::SequenceEffect>() {
            return crate::effect::Effect::new(rewrite_sequence(sequence));
        }
        if let Some(conditional) = effect.downcast_ref::<crate::effects::ConditionalEffect>() {
            let mut conditional = conditional.clone();
            conditional.if_true = conditional.if_true.iter().map(rewrite_effect).collect();
            conditional.if_false = conditional.if_false.iter().map(rewrite_effect).collect();
            return crate::effect::Effect::new(conditional);
        }
        effect.clone()
    }
    ...
    for segment in &mut program.segments {
        segment.default_effects = segment.default_effects.iter().map(rewrite_effect).collect();
...
/// A resolution-time player choice is the executable antecedent of later
/// actions written with singular `they`. Keep that identity instead of
/// widening the follow-up back to an arbitrary opponent.
fn bind_chosen_opponent_followup_actions(program: &mut crate::resolution::ResolutionProgram) {
    fn rewrite(effect: &crate::effect::Effect) -> crate::effect::Effect {
        if let Some(conditional) = effect.downcast_ref::<crate::effects::ConditionalEffect>() {
            let mut conditional = conditional.clone();
        ...
        let mut sequence = sequence.clone();
        sequence.effects = sequence.effects.iter().map(rewrite).collect();
        let mut choices = sequence.effects.iter().filter_map(|nested| {
            nested
                .downcast_ref::<crate::effects::ChoosePlayerEffect>()
                .filter(|choice| {
        ...
        let chosen_player = PlayerFilter::TaggedPlayer(choice.tag.clone());
        sequence.effects = sequence
            .effects
            .into_iter()
            .map(|nested| {
                let Some(scry) = nested.downcast_ref::<crate::effects::ScryEffect>() else {
                    return nested;
                };
                if scry.player != PlayerFilter::Opponent {
                    return nested;
                }
                let mut scry = scry.clone();
                scry.player = chosen_player.clone();
                crate::effect::Effect::new(scry)
            })
            .collect();
        crate::effect::Effect::new(sequence)
    }
...
/// A cast trigger can modify how the exact triggering permanent will enter
/// after the trigger resolves. Preserve the triggering spell's stable
/// identity across Stack -> Battlefield instead of putting counters on the
/// source card that the preceding sentence exiled.
pub(super) fn bind_graveyard_cast_trigger_to_triggering_permanent_entry(
    ability: &mut Ability,
    source_tokens: &[crate::lexer::OwnedLexToken],
) {
    let AbilityKind::Triggered(triggered) = &mut ability.kind else {
    ...
    if caster != &PlayerFilter::You
        || filter.card_types.as_slice() != [crate::types::CardType::Planeswalker]
        || filter.subtypes.as_slice() != [crate::types::Subtype::Bolas]
    {
        return;
    }
    if !source_tokens.is_empty() {
        ...
        if !words
            .windows(7)
            .any(|window| window == ["when", "you", "cast", "a", "bolas", "planeswalker", "spell"])
            || !words.windows(7).any(|window| {
                window
                    == [
                        "planeswalker",
                        "enters",
    ...
    let triggering_tag = crate::tag::TagKey::from("triggering_permanent_spell");
...
/// Preserve the actor separately from the library owner for reciprocal
/// inspection instructions. The generic subject/verb AST historically used
/// its player slot for the possessive library owner, which made the second
/// player silently lose the right to see and reorder the cards.
fn bind_reciprocal_library_inspection_actors(program: &mut crate::resolution::ResolutionProgram) {
    let [first_segment, second_segment] = program.segments.as_slice() else {
        return;
    };
    if !first_segment.self_replacements.is_empty()
        || !second_segment.self_replacements.is_empty()
        || first_segment.default_effects.len() != 1
        || second_segment.default_effects.len() != 1
    {
        return;
    }
    let Some(first) =
        first_segment.default_effects[0].downcast_ref::<crate::effects::SequenceEffect>()
    else {
        return;
    };
    let Some(second) =
        second_segment.default_effects[0].downcast_ref::<crate::effects::SequenceEffect>()
    else {
        return;
    };
    ...
    let Some(target) = target_effect.downcast_ref::<crate::effects::TargetOnlyEffect>() else {
        return;
    };
    ...
    let Some(first_look) = first_look_effect.downcast_ref::<crate::effects::LookAtTopCardsEffect>()
    else {
        return;
    };
    let Some(first_reorder) =
        first_reorder_effect.downcast_ref::<crate::effects::ReorderLibraryTopEffect>()
    else {
        return;
    };
    let Some(second_look) =
        second_look_effect.downcast_ref::<crate::effects::LookAtTopCardsEffect>()
    else {
        return;
    };
    let Some(second_reorder) =
        second_reorder_effect.downcast_ref::<crate::effects::ReorderLibraryTopEffect>()
    else {
        return;
    };
    if first_look.player != target_opponent
        ...
        || second_look.viewer != PlayerFilter::You
        || second_look.reveal
        || second_reorder.tag != second_look.tag
        || second_reorder.chooser != PlayerFilter::You
        || first_look.count != second_look.count
    {
        return;
    }

    let mut rebound_look = second_look.clone();
    rebound_look.viewer = target_opponent.clone();
    let mut rebound_reorder = second_reorder.clone();
    rebound_reorder.chooser = target_opponent;
    let mut rebound_sequence = second.clone();
    rebound_sequence.effects = vec![
        crate::effect::Effect::new(rebound_look),
        crate::effect::Effect::new(rebound_reorder),
    ];
    program.segments[1].default_effects[0] = crate::effect::Effect::new(rebound_sequence);
}
...
fn lower_optional_cost_chunk(
    ...
    let NormalizedLineChunk::OptionalCost(mut cost) = parsed else {
        unreachable!("optional-cost lowerer received mismatched chunk");
    };
    let authored_tokens =
        crate::lexer::lex_line(&info.raw_line, info.line_index).unwrap_or_default();
    let authored_words = crate::lexer::parser_token_word_refs(&authored_tokens);
        ...
        ]
    {
        ...
        let mut hand = ObjectFilter::creature()
            .owned_by(PlayerFilter::You)
            .in_zone(Zone::Hand);
        ...
        .with_description("Behold two creatures of that type");
        behold.reveal = true;
        cost.kind = crate::cost::OptionalCostKind::Additional;
        cost.reference =
            crate::cost::OptionalCostRef::new(crate::cost::OptionalCostKind::Additional);
        cost.source_label = info.raw_line.trim().to_string();
        cost.cost = crate::cost::TotalCost::from_costs(vec![
            crate::costs::Cost::validated_effect(crate::effect::Effect::choose_creature_type(
                PlayerFilter::You,
                vec![],
            )),
            crate::costs::Cost::validated_effect(crate::effect::Effect::new(behold)),
        ]);
    }
...
fn restore_authored_source_exiled_return(
    ...
    let Ok(parsed_return) =
        crate::effect_sentences::parse_effect_clause_lexed(&authored_tokens[return_index..])
    else {
        return Ok(false);
    };
