...
pub(super) fn parse_create_token_then_copy_spell_chain(
    ...
    let Some(then_idx) = crate::slice_primitives::select_position(tokens, |token| {
        token.is_word("then")
    }) else {
        return Ok(None);
    };
    ...
    let Some(copy_effect) = super::super::clause_pattern_helpers::parse_copy_spell_clause(&copy_tokens)?
    else {
        return Ok(None);
    };
    if !matches!(
