...
///
/// The generic coordinated-choice route treats the noun immediately after
/// `chooses` as an object domain.  In `chooses a creature type and ...`, that
/// would select a creature permanent and discard the terminal `type` noun.
/// Give the complete typed-choice phrase first refusal, then parse the
/// coordinated tail with the same participant actor.
pub(super) fn parse_participant_creature_type_choice_program(
