...
#[path = "counter_marker_shapes/followups.rs"]
mod followups;
pub use followups::*;
...
pub fn parse_created_token_distribution_target(tokens: &[OwnedLexToken]) -> bool {
    primitives::parse_all(
        ...
        (
            alt((
                primitives::phrase(&["either", "of", "those", "tokens"]),
                primitives::phrase(&["one", "of", "those", "tokens"]),
            )),
            primitives::sentence_end(),
        )
            .void(),
        "created-token counter distribution target",
    )
    .is_ok()
}
