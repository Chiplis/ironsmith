...
fn is_parley_word_program(words: &[&str]) -> bool {
...
fn is_gate_partition_core_word_program(words: &[&str]) -> bool {
    crate::word_primitives::sequence_occurs(
        words,
...
fn is_gate_partition_word_program(words: &[&str]) -> bool {
...
fn reconcile_authored_correlated_trigger_programs(
    ...
    if let Some(first_comma) = source_tokens
        .iter()
        .position(|token| token.kind == TokenKind::Comma)
    {
        let physical_tail = trim_lexed_commas(&source_tokens[first_comma + 1..]);
        if let Some(effects) =
...
use lines_condition_programs::{
    fixed_standard_gift_creature_definition, standard_gift_effects, standard_gift_followup_text,
};
