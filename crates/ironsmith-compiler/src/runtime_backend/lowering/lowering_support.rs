...
pub(crate) fn rewrite_prepare_triggered_effects_for_lowering(
    ...
    {
        bind_condition_antecedent_in_effects(&mut body_effects, &antecedent);
    }
...
fn effect_produces_mana(effect: &crate::effect::Effect) -> bool {
    ...
    if let Some(tagged) = effect.downcast_ref::<crate::effects::TaggedEffect>() {
        return effect_produces_mana(&tagged.effect);
    }
    if let Some(with_id) = effect.downcast_ref::<crate::effects::WithIdEffect>() {
        return effect_produces_mana(&with_id.effect);
    }
