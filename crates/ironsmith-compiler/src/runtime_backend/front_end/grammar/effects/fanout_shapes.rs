...
pub(crate) fn parse_same_name_fanout_shape(
    ...
    if let Some((deal_start, (), deal_rest)) = primitives::find_prefix(tokens, || deal_verb) {
        ...
        let deal_is_valid = deal_start == 0 || {
            let words = crate::runtime_backend::front_end::lexer::TokenWordView::new(source_tokens)
                .to_word_refs();
            crate::runtime_backend::front_end::shared::util::is_source_reference_words(&words)
        };
        if deal_is_valid
