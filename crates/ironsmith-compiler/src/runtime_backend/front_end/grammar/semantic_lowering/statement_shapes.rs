...
pub(crate) fn parse_self_counter_entry_tokens(
    ...
    let count = if revealed_total {
        Value::TotalManaValue(crate::target::ObjectFilter::tagged(crate::TagKey::from(
            "__public_revealed",
        )))
    } else {
        Value::X
...
fn parse_snow_condition(tokens: &[OwnedLexToken]) -> Option<ConditionExpr> {
    ...
    let symbol = crate::runtime_backend::grammar::values::parse_mana_symbol(body_words[0]).ok()?;
