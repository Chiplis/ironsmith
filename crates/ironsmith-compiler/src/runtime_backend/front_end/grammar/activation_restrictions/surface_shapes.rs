...
/// Winnow-backed description of one activation-restriction surface.
///
/// The family parser declares the surface vocabulary with this typed value; all
/// word-sequence recognition is executed by the grammar layer.
#[derive(Debug, Clone, Copy)]
pub(crate) struct RestrictionSurface<'p> {
...
impl<'p> RestrictionSurface<'p> {
    ...
    pub(crate) fn locate_window(self, words: &[&str], width: usize) -> Option<usize> {
        ...
        (0..=words.len() - width).find(|start| self.accepts_words(&words[*start..*start + width]))
    }
}

macro_rules! restriction_surface {
    (exact $expected:expr) => {
        $crate::runtime_backend::grammar::activation_restrictions::RestrictionSurface::new()
            .exact($expected)
    };
...
pub(crate) use restriction_surface;
...
mod tests {
    ...
    #[test]
    fn accepts_composed_restriction_surfaces() {
        let shape = restriction_surface!(prefix & ["cast", "spells"]; contains_words & ["turn"]);
        assert!(shape.accepts_words(&["cast", "spells", "during", "your", "turn"]));
        assert!(!shape.accepts_words(&["cast", "spells"]));
    }
}
