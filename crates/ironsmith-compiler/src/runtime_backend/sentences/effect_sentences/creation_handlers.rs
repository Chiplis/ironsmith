...
fn parse_create_for_each_counter_count(tokens: &[OwnedLexToken]) -> Option<Value> {
    ...
    let counter_type = words
        ...
        .or_else(|| {
            // Two-word named counter descriptor, e.g. "rally counter".
            if !words
                .get(idx + 1)
            ...
            let start = token_index_for_word_index(tokens, idx)?;
            let end = token_index_for_word_index(tokens, idx + 2)?;
            parse_counter_type_from_tokens(&tokens[start..end])
        });
