...
pub(in crate::compiled_text) fn describe_quantified_created_tokens_goaded_forever(
    effects: &[Effect],
) -> Option<String> {
    let [producer, consumer] = effects else {
        return None;
    };
    ...
    let [created] = players.effects.as_slice() else {
        return None;
    };
    ...
    let create = tagged
        ...
        .downcast_ref::<crate::effects::CreateTokenEffect>()?;
    let goad =
        structural_unwrap_render_wrappers(consumer).downcast_ref::<crate::effects::GoadEffect>()?;
