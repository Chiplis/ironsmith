...
fn hand_reveal_search_preserves_the_matching_name_and_empty_hand_alternative() {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Hand Search Probe")
...
fn hand_choice_reveal_requires_exact_matching_selection() {
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Reveal Probe")
        .card_types(vec![CardType::Sorcery]).parse_text("Reveal a card from your hand.").unwrap();
    ...
    let choice = effects[0].downcast_ref::<crate::effects::ChooseObjectsEffect>().unwrap();
    for variant in 0..6 {
        let mut changed = choice.clone();
        match variant {
            0 => changed.chooser = PlayerFilter::Opponent,
            1 => changed.filter.owner = Some(PlayerFilter::Opponent),
            2 => { changed.zone = Some(Zone::Graveyard); changed.filter.zone = Some(Zone::Graveyard); },
        ...
        assert!(describe_hand_choice_reveal_prefix(&altered).is_none(), "variant {variant}");
    }
}
