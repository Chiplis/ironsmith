...
fn source_attack_block_restrictions_track_hand_and_blessing_conditions() {
    for requirement in ["you have the city's blessing", "you have one or fewer cards in hand", "you have seven or more cards in hand"] {
        let oracle = format!("This creature can't attack or block unless {requirement}.");
        let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Conditional Combat Probe")
            .card_types(vec![CardType::Creature]).parse_text(&oracle).unwrap();
        assert!(definition.spell_effect.is_none(), "intrinsic restriction became spell effect: {requirement}");
        for count in [0, 1, 2, 6, 7, 8] {
            for blessed in [false, true] {
                ...
                for _ in 0..count { game.create_object_from_card(&card, alice, Zone::Hand); }
                game.grant_citys_blessing(bob);
                if blessed { game.grant_citys_blessing(alice); }
                ...
                assert_eq!(game.can_attack(source), allowed, "{requirement} count={count} blessed={blessed}");
