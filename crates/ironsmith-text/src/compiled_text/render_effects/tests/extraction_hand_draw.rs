...
impl crate::decision::DecisionMaker for SelectAll {
    fn decide_objects(&mut self, _: &crate::game_state::GameState,
        ctx: &crate::decisions::context::SelectObjectsContext) -> Vec<crate::ids::ObjectId> {
        ctx.candidates.iter().filter(|candidate| candidate.legal).map(|candidate| candidate.id).collect()
    }
}
...
fn check_extraction_hand_draw(permanent: bool) {
    ...
    let oracle = if permanent { permanent_oracle } else { spell_oracle };
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Extraction Probe")
        .card_types(vec![CardType::Instant]).parse_text(oracle).unwrap();
    fn has_outcome_tag(effect: &Effect) -> bool {
        if effect.downcast_ref::<crate::effects::TaggedEffect>().is_some_and(|tagged| tagged.outcome_only) { return true; }
        let mut found = false;
        effect.0.visit_child_effects(&mut |child| { found |= has_outcome_tag(child); });
        found
    }
    assert!(definition.spell_effect.as_ref().unwrap().segments.iter()
        .flat_map(|segment| &segment.default_effects).any(has_outcome_tag),
    ...
    for hand_count in [0, 1, 2] {
        for (redirected, redirect_all) in [(false, false), (true, false), (true, true)] {
            ...
            let matching = crate::card::CardBuilder::new(crate::ids::CardId::new(), "Matching Spell")
                .card_types(vec![if permanent { CardType::Creature } else { CardType::Instant }]).build();
            let spell = game.create_object_from_card(&matching, bob, if permanent { Zone::Battlefield } else { Zone::Stack });
            if !permanent { game.stack.push(crate::game_state::StackEntry::new(spell, bob)); }
            let mut hand_ids = Vec::new();
            for _ in 0..hand_count { hand_ids.push(game.create_object_from_card(&matching, bob, Zone::Hand)); }
            for zone in [Zone::Graveyard, Zone::Library] { game.create_object_from_card(&matching, bob, zone); }
            let other = crate::card::CardBuilder::new(crate::ids::CardId::new(), "Other Spell")
                .card_types(vec![CardType::Instant]).build();
            for _ in 0..6 { game.create_object_from_card(&other, bob, Zone::Library); }
            ...
            if redirected && let Some(id) = hand_ids.first() {
                let replacement = crate::effects::RegisterFutureZoneReplacementEffect::new(
                    if redirect_all { ObjectFilter::default().named("Matching Spell") } else { ObjectFilter::specific(*id) },
                    if redirect_all { None } else { Some(Zone::Hand) }, Some(Zone::Exile), Zone::Graveyard,
                    if redirect_all { crate::effects::ReplacementApplyMode::UntilEndOfTurn } else { crate::effects::ReplacementApplyMode::OneShot });
            ...
            assert_eq!(game.player(bob).unwrap().hand.len(), if redirect_all { 0 } else { hand_count - usize::from(redirected && hand_count > 0) }, "hand={hand_count}, redirected={redirected}, all={redirect_all}");
