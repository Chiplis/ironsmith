...
fn graveyard_cast_entry_counter_follows_the_cast_spell() {
    ...
    assert!(format!("{definition:#?}").contains("RegisterNextBatchEnterWithCountersEffect"), "{definition:#?}");
    for remove_source in [false, true] {
        ...
        if !remove_source { assert!(game.exile.iter().any(|id| game.object(*id).is_some_and(|o| o.name == "Dark Intimations"))); }
...
fn cast_entry_riders_preserve_generic_filters_and_counters() {
    for text in [
        ...
        let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Cast Entry Fixture")
            .card_types(vec![CardType::Sorcery]).parse_text(text).unwrap();
        let debug = format!("{definition:#?}");
        assert!(debug.contains("RegisterNextBatchEnterWithCountersEffect"), "{debug}");
...
fn ordinary_counter_placement_after_source_exile_is_not_an_entry_rider() {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Ordinary Counter Fixture")
        .card_types(vec![CardType::Sorcery]).parse_text(text).unwrap();
    let debug = format!("{definition:#?}");
...
fn graveyard_cast_trigger_preserves_caster_subtype_and_source_zone() {
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Cast Trigger Fixture")
        .card_types(vec![CardType::Sorcery]).parse_text(TEXT).unwrap();
    for (source_zone, opponent_cast, subtype, expected) in [
        ...
        let caster = if opponent_cast { game.players[1].id } else { alice };
        ...
        let walker = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Walker Fixture")
            .card_types(vec![CardType::Planeswalker]).subtypes(vec![subtype]).loyalty(4).build();
        ...
        let event = crate::triggers::TriggerEvent::new_with_provenance(
            ...
            crate::provenance::ProvNodeId::default());
        assert_eq!(crate::triggers::check_triggers(&game, &event).len(), expected,
