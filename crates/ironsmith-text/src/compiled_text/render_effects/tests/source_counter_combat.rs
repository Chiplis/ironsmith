...
fn intrinsic_counter_gate_uses_its_source_and_preserves_comparisons() {
    for subject in ["it", "this creature"] {
        for quantity in ["five or more", "exactly five", "two or fewer"] {
            let oracle = format!("This creature can't attack or block unless {subject} has {quantity} +1/+1 counters on it.");
            let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Counter Gate Probe")
            ...
            for count in [0, 1, 2, 3, 4, 5, 6, 2, 5] {
                ...
                let allowed = match quantity { "five or more" => count >= 5, "exactly five" => count == 5, _ => count <= 2 };
