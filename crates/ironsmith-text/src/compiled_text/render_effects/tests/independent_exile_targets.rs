...
fn independent_optional_exile_groups_are_targets_and_both_resolve() {
    ...
    for (choose_battlefield, choose_graveyard) in [(false, false), (true, false), (false, true), (true, true)] {
        ...
        let targets = [(choose_battlefield, battlefield), (choose_graveyard, graveyard)]
            .into_iter().filter(|(choose, _)| *choose)
            .map(|(_, id)| crate::effects::ResolvedTarget::Object(id)).collect();
        ...
        for (chosen, id) in [(choose_battlefield, battlefield), (choose_graveyard, graveyard)] {
            assert_eq!(game.object(id).is_none(), chosen, "battlefield={choose_battlefield}, graveyard={choose_graveyard}");
        }
        assert_eq!(game.exile.len(), usize::from(choose_battlefield) + usize::from(choose_graveyard));
    }
}
...
fn independent_exiles_return_to_each_owner_after_the_source_dies() {
    for source_reference in ["it", "this creature"] {
        for source_move in 0..3 { check_linked_exile_return(source_reference, source_move); }
    }
}
...
fn check_linked_exile_return(source_reference: &str, source_move: usize) {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Linked Exile Probe")
        .card_types(vec![CardType::Creature]).parse_text(&oracle).unwrap();
    let abilities: Vec<_> = definition.abilities.iter().filter_map(|ability| {
        if let crate::ability::AbilityKind::Triggered(triggered) = &ability.kind { Some(triggered) } else { None }
    }).collect();
    ...
    let mut ctx = crate::effects::EffectContext::new_default(source, alice).with_triggering_event(entry)
        .with_targets(vec![crate::effects::ResolvedTarget::Object(battlefield), crate::effects::ResolvedTarget::Object(graveyard)]);
    ctx.snapshot_targets(&game);
    ...
    let dead_source = game.move_object(source, Zone::Graveyard, crate::events::cause::EventCause::effect()).unwrap();
    if source_left_graveyard {
        let exiled_source = game.move_object(dead_source, Zone::Exile, crate::events::cause::EventCause::effect()).unwrap();
        if source_move == 2 { game.move_object(exiled_source, Zone::Graveyard, crate::events::cause::EventCause::effect()).unwrap(); }
    }
    let death = crate::triggers::TriggerEvent::new_with_provenance(
        crate::events::ZoneChangeEvent::with_results(source, vec![dead_source], Zone::Battlefield, Zone::Graveyard,
            crate::events::cause::EventCause::effect(), Some(snapshot.clone())),
        crate::provenance::ProvNodeId::default());
    game.stack.push(crate::game_state::StackEntry::ability(source, alice, abilities[1].effects.clone())
        .with_source_snapshot(snapshot).with_triggering_event(death));
    crate::game_loop::resolve_stack_entry(&mut game).unwrap();
    let source_now = game.find_object_by_stable_id(source_stable).unwrap();
    assert_eq!(game.object(source_now).unwrap().zone, match source_move { 0 => Zone::Library, 1 => Zone::Exile, _ => Zone::Graveyard }, "a source that left its death zone is a new object");
    ...
    if !source_left_graveyard { assert_eq!(game.player(alice).unwrap().library.first(), Some(&returned_source)); }
}
