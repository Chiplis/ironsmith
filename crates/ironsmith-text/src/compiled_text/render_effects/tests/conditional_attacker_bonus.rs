...
fn conditional_attacker_bonus_includes_already_untapped_attackers() {
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Motivated Pony")
        .card_types(vec![CardType::Creature]).subtypes(vec![Subtype::Horse])
        .power_toughness(crate::card::PowerToughness::fixed(3, 3)).parse_text(TEXT).unwrap();
    let ability = definition.abilities.iter().find_map(|ability| match &ability.kind { AbilityKind::Triggered(trigger) => Some(trigger), _ => None }).unwrap();
    for food_owner in [None, Some(0), Some(1)] {
        ...
        combat.attackers = [source, tapped, untapped].into_iter().map(|creature| crate::combat_state::AttackerInfo {
            creature, target: crate::combat_state::AttackTarget::Player(bob),
        }).collect();
        ...
        if let Some(index) = food_owner {
            ...
            let food = crate::card::CardBuilder::new(crate::ids::CardId::new(), "Food")
                .card_types(vec![CardType::Artifact]).subtypes(vec![Subtype::Food]).build();
        ...
        for id in [source, tapped, untapped] {
            ...
            assert_eq!(game.calculated_power(id), Some(base + bonus), "food={food_owner:?}, object={id:?}");
        ...
        for id in [tapped, untapped, nonattacker] { assert_eq!(game.calculated_power(id), Some(2)); }
    }
}
#[test]
...
fn conditional_attacker_bonus_compaction_requires_the_entire_original_group() {
    ...
    for change in 0..4 {
        ...
        let mut conditional = effects[1].downcast_ref::<crate::effects::ConditionalEffect>().unwrap().clone();
        let mut sequence = conditional.if_true[0].downcast_ref::<crate::effects::SequenceEffect>().unwrap().clone();
        match change {
            0 => {
                let mut capture = sequence.effects[0].downcast_ref::<crate::effects::TagMatchingObjectsEffect>().unwrap().clone();
                capture.filter.tapped = true;
                sequence.effects[0] = Effect::new(capture);
            }
            1 => {
                let mut untap = sequence.effects[1].downcast_ref::<crate::effects::UntapEffect>().unwrap().clone();
                untap.target = ChooseSpec::Source;
                sequence.effects[1] = Effect::new(untap);
            }
            2 => {
                let mut bonus = sequence.effects[2].downcast_ref::<crate::effects::ApplyContinuousEffect>().unwrap().clone();
        ...
        assert!(describe_group_pump_then_conditional_extra_bonus(&effects).is_none(), "change {change}");
    }
}
...
fn conditional_attacker_bonus_renders_structured_amounts() {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Different group bonus")
        .card_types(vec![CardType::Creature]).parse_text(&text).unwrap();
    assert_eq!(crate::compiled_text::compiled_text_lines(&definition).join("\n"), text);
}
