...
fn conditional_quoted_grant_depends_on_defending_players_actual_sacrifice() {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Conditional Grant Probe")
        .card_types(vec![CardType::Creature]).power_toughness(crate::card::PowerToughness::fixed(3, 1))
        .parse_text(oracle).unwrap();
    let rendered = crate::compiled_text::compiled_text_lines(&definition).join("\n");
    assert!(rendered.contains("\" until end of turn unless defending player sacrifices a creature"), "{rendered}");
    let crate::ability::AbilityKind::Triggered(triggered) = &definition.abilities[0].kind else { panic!("expected attack trigger") };
    for accepts in [false, true] {
        for available in [false, true] {
            ...
            let creature = crate::card::CardBuilder::new(crate::ids::CardId::new(), "Payment Creature")
                .card_types(vec![CardType::Creature]).build();
            let other = game.create_object_from_card(&creature, carol, Zone::Battlefield);
            if available { game.create_object_from_card(&creature, bob, Zone::Battlefield); }
            let event = crate::triggers::TriggerEvent::new(
                crate::events::combat::CreatureAttackedEvent::new(source, crate::triggers::event::AttackEventTarget::Player(bob)),
                crate::provenance::ProvNodeId::default(),
            );
            let mut decision = SacrificeDecision(accepts);
            let mut ctx = crate::effects::EffectContext::new_default(source, alice)
                .with_decision_maker(&mut decision).with_defending_player(bob).with_triggering_event(event);
            ...
            assert_eq!(game.can_be_blocked(source), paid, "accepts={accepts}, available={available}");
