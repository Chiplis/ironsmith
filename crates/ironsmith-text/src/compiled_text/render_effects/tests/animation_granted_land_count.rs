...
fn animation_granted_land_count_keeps_types_and_dynamic_size() {
    let definition=crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Beorn's Hospitality")
    ...
    let land=crate::card::CardBuilder::new(crate::ids::CardId::new(),"Test Land").card_types(vec![CardType::Land]).build();
    for _ in 0..3 { game.create_object_from_card(&land,alice,Zone::Battlefield); }
    for _ in 0..5 { game.create_object_from_card(&land,bob,Zone::Battlefield); }
    let crate::ability::AbilityKind::Activated(ability)=&definition.abilities[0].kind else {panic!("activated");};
    ...
    let remove = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Ability Removal")
        .card_types(vec![CardType::Instant]).parse_text("Target creature loses all abilities until end of turn.").unwrap();
    ...
    for segment in &program.segments { for effect in &segment.default_effects {crate::effects::execute_effect(&mut game,effect,&mut ctx).unwrap();} }
    assert!(!game.current_has_static_ability_id(source,crate::static_abilities::StaticAbilityId::CharacteristicDefiningPT));
    assert_ne!(game.current_power(source),Some(3),"removed grant cannot keep setting size");
    game.effect_store.continuous_effects.cleanup_end_of_turn();game.refresh_continuous_state();
    assert_eq!((game.current_power(source),game.current_toughness(source)),(Some(3),Some(3)));
    ...
    let setting=crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Size Setting")
        .card_types(vec![CardType::Instant]).parse_text("Target creature has base power and toughness 1/1 until end of turn.").unwrap();
    ...
    ctx.snapshot_targets(&game);
    for segment in &program.segments { for effect in &segment.default_effects {crate::effects::execute_effect(&mut game,effect,&mut ctx).unwrap();} }
    assert_eq!(game.current_power(source),Some(1),"setting before second grant");
    ...
    for segment in &ability.effects.segments { for effect in &segment.default_effects {crate::effects::execute_effect(&mut game,effect,&mut ctx).unwrap();} }
    assert_eq!((game.current_power(source),game.current_toughness(source)),(Some(1),Some(1)));
    assert_eq!((game.current_power(second),game.current_toughness(second)),(Some(3),Some(3)));
}
