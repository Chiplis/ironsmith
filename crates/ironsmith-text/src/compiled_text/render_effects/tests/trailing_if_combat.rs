...
fn trailing_if_combat_restriction_tracks_battlefield_presence() {
    ...
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Presence Gate Probe")
    ...
    for opponent_controls in [false, true] {
        ...
        let mut object = game.create_object_from_card(&enchantment, if opponent_controls { bob } else { alice }, Zone::Hand);
        for zone in [Zone::Hand, Zone::Battlefield, Zone::Graveyard, Zone::Battlefield, Zone::Exile] {
            if game.object(object).unwrap().zone != zone { object = game.move_object_by_effect(object, zone).unwrap(); }
...
fn trailing_if_counter_condition_remains_bound_to_source() {
    let definition = crate::CardDefinitionBuilder::new(crate::ids::CardId::new(), "Counter If Probe")
        .card_types(vec![CardType::Creature]).parse_text("This creature can't attack or block if it has five or more +1/+1 counters on it.").unwrap();
    assert!(definition.spell_effect.is_none());
    let mut game = crate::game_state::GameState::new(vec!["Alice".into(), "Bob".into()], 20);
    let alice = game.players[0].id;
    let bob = game.players[1].id;
    let source = game.create_object_from_definition(&definition, alice, Zone::Battlefield);
    let card = crate::card::CardBuilder::new(crate::ids::CardId::new(), "Other").card_types(vec![CardType::Creature]).build();
