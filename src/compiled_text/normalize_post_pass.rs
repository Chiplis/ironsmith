...
fn normalize_reveal_until_put_hand_graveyard_rest_surface(text: &str) -> Option<String> {
    ...
    let after = &text[stop_end + tail_marker.len()..];
    let mut rewritten = format!(
        "{before}Reveal cards from the top of your library until you reveal {stop_text}. Put that card into your hand and all other cards revealed this way into your graveyard."
    );
    let trailing = after.trim();
    if let Some(rest) = trailing.strip_prefix('.') {
        let rest = rest.trim();
...
pub(super) fn normalize_compiled_post_pass_effect(text: &str) -> String {
    ...
    if let Some(rewritten) = normalize_reveal_until_put_hand_graveyard_rest_surface(&normalized) {
        normalized = rewritten;
    }
    loop {
