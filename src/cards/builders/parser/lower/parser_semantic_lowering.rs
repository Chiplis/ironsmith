use super::*;
use super::super::rewrite_exceptions;
...
fn lower_rewrite_statement_to_chunks_impl(
    ...
    if let Some(chunks) = rewrite_exceptions::apply_statement_rewrite_exceptions(line, parse_tokens)?
    {
        return Ok(chunks);
    }
...
pub(crate) fn lower_rewrite_statement_to_unsupported_chunk(
    line: &RewriteStatementLine,
) -> Option<LineAst> {
...
pub(crate) fn lower_rewrite_soul_partition_statement_to_chunk(
    line: &RewriteStatementLine,
    parse_tokens: &[OwnedLexToken],
...
pub(super) fn parse_single_effect_lexed(tokens: &[OwnedLexToken]) -> Result<EffectAst, CardTextError> {
    parse_effect_sentences_lexed(tokens)?
        .into_iter()
        .next()
...
pub(crate) fn lower_rewrite_divvy_statement_to_chunk(
    line: &RewriteStatementLine,
    parse_tokens: &[OwnedLexToken],
) -> Result<Option<LineAst>, CardTextError> {
    let normalized = line.text.trim().to_ascii_lowercase();
...
pub(crate) fn lower_rewrite_empty_laboratory_statement_to_chunk(
    line: &RewriteStatementLine,
...
pub(crate) fn lower_rewrite_shape_anew_statement_to_chunk(
    line: &RewriteStatementLine,
    _parse_tokens: &[OwnedLexToken],
) -> Result<Option<LineAst>, CardTextError> {
    let normalized = line.text.trim().to_ascii_lowercase();
...
pub(crate) fn lower_rewrite_nissas_encouragement_statement_to_chunk(
    line: &RewriteStatementLine,
    _parse_tokens: &[OwnedLexToken],
) -> Result<Option<LineAst>, CardTextError> {
    let normalized = line.text.trim().to_ascii_lowercase();
...
fn lower_rewrite_triggered_to_chunk_impl(
    ...
    if let Some(chunk) = rewrite_exceptions::apply_triggered_rewrite_exceptions(
        line,
        trigger_parse_tokens,
        effect_parse_tokens,
    )? {
...
pub(crate) fn lower_keyword_special_cases(
    ...
    rewrite_exceptions::apply_keyword_rewrite_exceptions(line, parse_tokens)
}
...
pub(crate) fn lower_rewrite_pact_statement_to_chunk(
    line: &RewriteStatementLine,
    parse_tokens: &[OwnedLexToken],
) -> Result<Option<LineAst>, CardTextError> {
