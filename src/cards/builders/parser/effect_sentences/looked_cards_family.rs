...
use super::search_library::{normalize_search_library_filter, parse_search_library_disjunction_filter};
use crate::cards::builders::{CardTextError, EffectAst, ObjectFilter, PlayerAst, TagKey, TextSpan};
use crate::cards::builders::IT_TAG;
use crate::cards::builders::parser::grammar::primitives::TokenWordView;
...
pub(crate) fn parse_top_cards_view_sentence(
    ...
    Some((PlayerAst::You, crate::effect::Value::Fixed(count as i32), revealed))
}
...
pub(crate) fn parse_looked_card_reveal_filter(tokens: &[OwnedLexToken]) -> Option<ObjectFilter> {
    ...
    let mut filter = parse_search_library_disjunction_filter(&filter_tokens)
        .or_else(|| super::super::object_filters::parse_object_filter(&filter_tokens, false).ok())?;
    if same_name_suffix_len.is_some() {
        filter = filter.match_tagged(
            TagKey::from(CHOSEN_NAME_TAG),
            TaggedOpbjectRelation::SameNameAsTagged,
