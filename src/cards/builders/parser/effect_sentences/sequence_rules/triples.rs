...
use super::super::dispatch_entry::{
    consult_cast_effects, parse_bargained_face_down_cast_mana_value_gate,
    ...
    parse_top_cards_view_sentence, consult_stop_rule_is_single_match, ConsultCastCost,
};
...
use crate::cards::builders::{CardTextError, EffectAst, IfResultPredicate, ObjectFilter, PredicateAst, TagKey, TargetAst};
use crate::effect::Value;
use crate::target::ChooseSpec;
use crate::zone::Zone;
use crate::cards::builders::parser::activation_and_restrictions::find_word_sequence_start;
use crate::cards::builders::parser::util::is_article;

pub(super) fn parse_mill_then_may_put_from_among_into_hand_then_if_you_dont(
    sentences: &[SentenceInput],
    sentence_idx: usize,
) -> Result<Option<Vec<EffectAst>>, CardTextError> {
    let Some(mut effects) = super::pairs::parse_mill_then_may_put_from_among_into_hand(
        sentences,
        sentence_idx,
    )?
    else {
...
pub(crate) fn parse_top_cards_for_each_card_type_among_spells_put_matching_into_hand_rest_bottom(
    ...
    let filter_token_end = crate::cards::builders::parser::token_index_for_word_index(
        &filter_tokens,
        filter_word_len,
    )
    .unwrap_or(filter_tokens.len());
    let filter_prefix_tokens = trim_commas(&filter_tokens[..filter_token_end]);
    let mut spell_filter = crate::cards::builders::parser::parse_spell_filter_lexed(
...
pub(super) fn parse_look_at_top_reveal_match_put_rest_bottom(
    ...
    let Ok(first_effects) = effect_sentences::parse_effect_sentence_lexed(
    ...
    let second_tokens = trim_commas(sentences[sentence_idx + 1].lowered());
    let Some(action_match) =
        parse_leading_may_action_lexed(&second_tokens, &["reveal"], true)
    else {
        return Ok(None);
    };
    let chooser = effect_sentences::leading_may_actor_to_player(action_match.actor, *player);
    ...
    let mut filter = if let Some(filter) =
        effect_sentences::parse_looked_card_reveal_filter(&filter_tokens)
    {
        filter
    } else {
        return Ok(None);
    ...
    effects.push(EffectAst::ChooseFromLookedCardsIntoHandRestOnBottomOfLibrary {
        player: chooser,
        filter,
        reveal: true,
        if_not_chosen: Vec::new(),
...
pub(super) fn parse_prefix_then_consult_match_move_and_bottom_remainder(
    ...
    let Ok(prefix_effects) = effect_sentences::parse_effect_sentence_lexed(
        sentences[sentence_idx].lowered(),
    )
