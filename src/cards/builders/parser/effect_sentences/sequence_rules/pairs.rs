...
use crate::cards::builders::parser::activation_and_restrictions::contains_word_sequence;
use super::super::dispatch_entry::{
    consult_cast_effects, consult_stop_rule_is_single_match,
    find_from_among_looked_cards_phrase, leading_may_actor_to_player,
    parse_consult_cast_clause, parse_consult_remainder_order,
...
use crate::cards::builders::parser::token_primitives::{
    find_index, parse_leading_may_action_lexed, slice_contains, slice_ends_with,
    slice_starts_with, word_view_has_any_prefix,
};
use crate::cards::builders::parser::token_index_for_word_index;
use crate::cards::builders::parser::util::{helper_tag_for_tokens, is_article};
use crate::cards::builders::parser::util::trim_commas;
use crate::cards::builders::{
    CardTextError, ChoiceCount, EffectAst, ObjectFilter, OwnedLexToken, PlayerAst,
    PredicateAst, ReturnControllerAst, TagKey, TargetAst,
};
...
fn parse_may_put_filtered_card_from_among_into_hand(
    ...
    let Some(action_match) = parse_leading_may_action_lexed(&sentence_tokens, &["put"], true) else {
        return Ok(None);
    };
...
pub(super) fn parse_choose_then_do_same_for_filter_then_return_to_battlefield(
    ...
    let Some(mut effects) =
        effect_sentences::parse_sentence_choose_then_do_same_for_filter(
    ...
    let second_words: Vec<&str> =
        crate::cards::builders::parser::token_word_refs(sentences[sentence_idx + 1].lowered())
            .into_iter()
...
pub(super) fn parse_delayed_dies_exile_top_power_choose_play(
    ...
    if crate::cards::builders::parser::grammar::primitives::words_match_prefix(
        &first_tokens,
...
pub(super) fn parse_target_gains_flashback_until_eot_with_targets_mana_cost(
    ...
    let Some(gain_idx) = find_index(&first_words, |word| matches!(*word, "gain" | "gains"))
    else {
        return Ok(None);
    };
    ...
    let Some(gain_token_idx) = token_index_for_word_index(&first_tokens, gain_idx)
    else {
        return Ok(None);
    };
...
pub(super) fn parse_consult_match_move_and_bottom_remainder(
    ...
    let second_words = crate::cards::builders::parser::token_word_refs(&second_tokens);
    let (zone, battlefield_tapped) = if crate::cards::builders::parser::grammar::primitives::words_match_prefix(
        &second_tokens,
        &["put", "that", "card", "into", "your", "hand"],
    )
    .is_some()
        || crate::cards::builders::parser::grammar::primitives::words_match_prefix(
            &second_tokens,
    ...
    {
        (Zone::Hand, false)
    } else if crate::cards::builders::parser::grammar::primitives::words_match_prefix(
        &second_tokens,
    ...
    if !crate::cards::builders::parser::grammar::primitives::contains_word(&second_tokens, "rest")
        && !crate::cards::builders::parser::grammar::primitives::contains_word(&second_tokens, "other")
    {
        return Ok(None);
    }
...
pub(super) fn parse_consult_match_into_hand_exile_others(
    ...
    let second_tokens = trim_commas(second);
    let moves_to_hand = crate::cards::builders::parser::grammar::primitives::words_match_prefix(
        &second_tokens,
        &["put", "that", "card", "into", "your", "hand"],
    )
    .is_some()
        ...
        .is_some();
    let exiles_rest = crate::cards::builders::parser::grammar::primitives::contains_word(
        &second_tokens,
        "exile",
    ...
    effects.push(EffectAst::ForEachTagged {
        ...
        effects: vec![EffectAst::Conditional {
            ...
            if_false: vec![EffectAst::Exile {
                target: TargetAst::Tagged(crate::cards::builders::TagKey::from(crate::cards::builders::IT_TAG), None),
                face_down: false,
            }],
        }],
    });
