...
fn anthem_last_token_offset(
    ...
    crate::cards::builders::compiler::grammar::primitives::rfind_token_index(tokens, |token| {
        predicate(token)
    })
}
...
type AnthemNormalizedWords<'a> = crate::cards::builders::compiler::grammar::primitives::TokenWordView<'a>;
...
fn triggered_grant_effects_and_condition(
        ...
        && if_false.is_empty()
    {
        ...
        imports.last_player_filter =
            crate::cards::builders::compiler::compile_support::inferred_trigger_player_filter(
                trigger,
            );
        let reference_env =
            crate::cards::builders::compiler::reference_model::ReferenceEnv::from_imports(
                &imports,
                false,
                false,
                false,
        ...
        let condition = crate::cards::builders::compiler::compile_support::compile_condition_from_predicate_ast_with_env(
            predicate,
            &reference_env,
            None,
        )?;
...
pub(crate) fn parse_subject_cant_be_blocked_line(
    ...
    if anthem_has_word_sequence(&subject_words, &["power", "or", "toughness"])
        || anthem_has_word_sequence(&subject_words, &["toughness", "or", "power"])
    {
...
pub(crate) fn parse_subject_has_keywords_and_cant_be_blocked_line(
    ...
    let ability_words = crate::cards::builders::compiler::token_word_refs(&ability_tokens);
    let Some(and_idx) = anthem_find_word_index(&ability_words, |word| word == "and") else {
        return Ok(None);
    };
...
pub(crate) fn parse_subject_cant_be_blocked_as_long_as_condition_line(
    ...
    let Some(cant_idx) = anthem_find_word_sequence_index(&normalized, &["cant", "be", "blocked"])
    else {
        return Ok(None);
    };

    let tail = &normalized[cant_idx + 3..];
    if !anthem_word_slice_starts_with(tail, &["as", "long", "as"]) {
        return Ok(None);
    }
...
pub(crate) fn parse_granted_keyword_static_line(
    ...
    fn parse_granted_escape_cost_tail(
        ...
        let tail = crate::cards::builders::compiler::token_word_refs(
    ...
    fn parse_granted_alternative_cast_static(
        ...
        let keyword_words = crate::cards::builders::compiler::token_word_refs(keyword_tokens);
    ...
    if crate::cards::builders::compiler::token_word_refs(&tokens[..have_token_idx])
        .iter()
    ...
    let tail_sentences =
        crate::cards::builders::compiler::grammar::primitives::split_lexed_slices_on_period(
            &tail_tokens,
        );
    ...
    .or_else(|| {
        anthem_find_word_sequence_index(
            &keyword_words,
...
pub(crate) fn parse_each_creature_cant_be_blocked_by_more_than_line(
    ...
    let Some(cant_idx) = anthem_find_word_sequence_index(&clause_words, &["cant", "be", "blocked"])
    else {
        return Ok(None);
    };
    if cant_idx < 2 || !anthem_word_slice_starts_with(&clause_words, &["each", "creature"]) {
        return Ok(None);
    }
    let tail = &clause_words[cant_idx..];
    if !anthem_word_slice_starts_with(tail, &["cant", "be", "blocked", "by", "more", "than"]) {
        return Ok(None);
    }
...
pub(crate) fn parse_each_creature_can_block_additional_creature_each_combat_line(
    ...
    let (_subject_len, you_control) = if anthem_word_slice_starts_with(
        &clause_words,
        &["each", "creature", "you", "control", "can", "block"],
    ) {
        (4usize, true)
    } else if anthem_word_slice_starts_with(&clause_words, &["each", "creature", "can", "block"]) {
        (2usize, false)
    } else {
        return Ok(None);
    };
    ...
    let Some(additional_word_idx) =
        anthem_find_word_index(&clause_words, |word| word == "additional")
    else {
        return Ok(None);
    };
...
pub(crate) fn parse_lose_all_abilities_and_transform_base_pt_line(
    ...
    let Some(is_idx) = anthem_find_word_index(&words, |word| matches!(word, "is" | "are")) else {
        return Ok(None);
    };
    let Some(with_idx) =
        anthem_find_word_sequence_index(&words, &["with", "base", "power", "and", "toughness"])
    else {
        return Ok(None);
    };
    if with_idx <= is_idx {
    ...
    let has_lose_all = anthem_has_word_sequence(&words, &["lose", "all", "abilities"])
        || anthem_has_word_sequence(&words, &["loses", "all", "abilities"]);
    if !has_lose_all {
        return Ok(None);
    }

    let subject_end = is_idx.min(
        anthem_find_word_index(&words, |word| matches!(word, "lose" | "loses")).unwrap_or(is_idx),
    );
    if subject_end == 0 {
        return Ok(None);
    }
    ...
    if let Some(named_idx) = anthem_find_word_index(tail_words, |word| word == "named") {
        let end_idx = anthem_find_word_index(&tail_words[named_idx + 1..], |word| {
            matches!(
                word,
    ...
    let has_except_mana = anthem_has_word_sequence(&words, &["except", "mana", "abilities"]);
...
pub(crate) fn parse_lose_all_abilities_and_base_pt_line(
    ...
    if let Some(have_idx) = have_idx {
        ...
        if anthem_word_slice_starts_with(after_have, &["base", "power", "and", "toughness"])
            && let Some(modifier_token_idx) =
                anthem_find_word_index(after_have, |word| word.chars().any(|ch| ch == '/'))
...
pub(crate) fn parse_best_object_filter_suffix(tokens: &[OwnedLexToken]) -> Option<ObjectFilter> {
    ...
    for start in 0..tokens.len() {
        ...
        let candidate_words = crate::cards::builders::compiler::token_word_refs(candidate);
...
pub(crate) fn parse_anthem_subject(
    ...
    let subject_words = crate::cards::builders::compiler::token_word_refs(tokens);
...
fn infer_attached_subject_filter_from_condition_tokens(
    ...
    let condition_words = crate::cards::builders::compiler::token_word_refs(&condition_tokens);
...
fn parse_anthem_subject_with_attached_fallback(
    ...
    if crate::cards::builders::compiler::token_word_refs(tokens).as_slice() == ["it"]
...
pub(crate) fn parse_permanent_card_count_filter(tokens: &[OwnedLexToken]) -> Option<ObjectFilter> {
    let token_words = crate::cards::builders::compiler::token_word_refs(tokens);
...
pub(crate) fn parse_static_condition_clause(
    ...
    if (anthem_word_slice_starts_with(&clause_words, &["youve", "completed"])
        && clause_words.len() > 2)
        || (anthem_word_slice_starts_with(&clause_words, &["you", "have", "completed"])
            && clause_words.len() > 3)
    {
    ...
    if let Some(is_idx) = anthem_find_word_index(&clause_words, |word| matches!(word, "is" | "are"))
    {
    ...
    if anthem_word_slice_starts_with(&clause_words, &["there", "are"])
        || anthem_word_slice_starts_with(&clause_words, &["there", "is"])
    {
    ...
    let control_prefix_len = if anthem_word_slice_starts_with(&clause_words, &["you", "control"])
        || anthem_word_slice_starts_with(&clause_words, &["you", "controls"])
        || anthem_word_slice_starts_with(&clause_words, &["opponent", "control"])
        || anthem_word_slice_starts_with(&clause_words, &["opponent", "controls"])
        || anthem_word_slice_starts_with(&clause_words, &["opponents", "control"])
        || anthem_word_slice_starts_with(&clause_words, &["opponents", "controls"])
    {
        2
    } else if anthem_word_slice_starts_with(&clause_words, &["an", "opponent", "control"])
        || anthem_word_slice_starts_with(&clause_words, &["an", "opponent", "controls"])
        || anthem_word_slice_starts_with(&clause_words, &["your", "opponents", "control"])
        || anthem_word_slice_starts_with(&clause_words, &["your", "opponents", "controls"])
    {
        3
    } else {
        0
    ...
    let own_prefix_len = if anthem_word_slice_starts_with(&clause_words, &["you", "own"])
        || anthem_word_slice_starts_with(&clause_words, &["you", "owns"])
        || anthem_word_slice_starts_with(&clause_words, &["opponent", "own"])
        || anthem_word_slice_starts_with(&clause_words, &["opponent", "owns"])
        || anthem_word_slice_starts_with(&clause_words, &["opponents", "own"])
        || anthem_word_slice_starts_with(&clause_words, &["opponents", "owns"])
    {
        2
    } else if anthem_word_slice_starts_with(&clause_words, &["an", "opponent", "own"])
        || anthem_word_slice_starts_with(&clause_words, &["an", "opponent", "owns"])
        || anthem_word_slice_starts_with(&clause_words, &["your", "opponents", "own"])
        || anthem_word_slice_starts_with(&clause_words, &["your", "opponents", "owns"])
    {
        3
    } else {
        0
    ...
    if anthem_contains_word(&clause_words, "entered")
        && let Ok(filter) = parse_object_filter(
            &tokens,
            matches!(clause_words.first().copied(), Some("another" | "other")),
        )
        && (filter.entered_battlefield_this_turn || filter.entered_battlefield_controller.is_some())
    {
        return Ok(crate::ConditionExpr::CountComparison {
            count: AnthemCountExpression::MatchingFilter(filter),
            comparison: crate::effect::Comparison::GreaterThan(0),
            display: Some(clause_words.join(" ")),
        });
    }
    ...
    let has_counter_on_source = anthem_has_word_sequence(&clause_words, &["on", "it"])
        || anthem_has_word_sequence(&clause_words, &["on", "this"])
        || anthem_has_word_sequence(&clause_words, &["on", "him"])
        || anthem_has_word_sequence(&clause_words, &["on", "her"]);
    if has_counter_on_source
        && let Some(has_idx) =
            anthem_find_word_index(&clause_words, |word| matches!(word, "has" | "have"))
    {
            ...
            && (is_source_reference_words(subject_words) || source_pronoun_subject)
        {
            ...
            let counter_words = crate::cards::builders::compiler::token_word_refs(counter_tokens);
            ...
            let on_source_tail = anthem_word_slice_starts_with(tail, &["on", "it"])
                || anthem_word_slice_starts_with(tail, &["on", "this"])
                || anthem_word_slice_starts_with(tail, &["on", "him"])
                || anthem_word_slice_starts_with(tail, &["on", "her"]);
            if !on_source_tail {
                return Err(CardTextError::ParseError(format!(
...
pub(crate) fn parse_anthem_for_each_expression(
    ...
    if let Some(attached_idx) = anthem_token_offset(rest, |token| token.is_word("attached")) {
        ...
        let tail_words = crate::cards::builders::compiler::token_word_refs(&rest[attached_idx + 1..]);
...
pub(crate) fn parse_type_color_addition_clause(
    ...
    let Some(addition_idx) =
        anthem_find_word_sequence_index(&words, &["in", "addition", "to", "its", "other"])
    else {
        return Ok(None);
    };
    if addition_idx <= 1 {
...
pub(crate) fn parse_soulbond_shared_line(
    ...
    let Some(paired_word_idx) = anthem_find_word_sequence_index(
        &clause_words,
    ...
    let pt_modifier_idx =
        if anthem_word_slice_starts_with(&rest_words, &["both", "creatures", "get"]) {
            Some(3usize)
        } else if anthem_word_slice_starts_with(
            &rest_words,
    ...
    let ability_start =
        if anthem_word_slice_starts_with(&rest_words, &["both", "creatures", "have"]) {
            Some(3usize)
        } else if anthem_word_slice_starts_with(
            &rest_words,
...
pub(crate) fn parse_anthem_and_keyword_line(
    ...
    let pre_grant_words =
        crate::cards::builders::compiler::token_word_refs(&tokens[..have_token_idx]);
    ...
    if let Some(as_long_idx) = anthem_find_word_sequence_index(
        &crate::cards::builders::compiler::token_word_refs(&ability_tokens),
        &["as", "long", "as"],
    ) {
        let as_token_idx =
...
fn every_subtype_family_for_subject(
    ...
    let ability = condition
        .as_ref()
        .and_then(|cond| base.with_condition(cond.clone()))
        .unwrap_or(base);
...
fn parse_triggered_granted_ability(
    ...
    let ability = match crate::cards::builders::compiler::clause_support::parse_triggered_line_lexed(
        &trigger_tokens,
    )? {
        LineAst::Triggered {
            trigger,
            ...
            parsed_triggered_ability(
                ...
                Some(crate::cards::builders::compiler::token_word_refs(&trigger_tokens).join(" ")),
                intervening_if,
    ...
    if parsed_triggered_ability_is_empty(&ability) {
        return Err(CardTextError::ParseError(format!(
            ...
            crate::cards::builders::compiler::token_word_refs(&trigger_tokens).join(" ")
        )));
    }
    Ok(Some(ability))
}
...
fn split_anthem_trailing_segments_preserving_granted_abilities(
    ...
    while idx < tokens.len() {
        ...
        let segment_words = crate::cards::builders::compiler::token_word_refs(&trimmed)
            .into_iter()
...
fn parse_granted_keyword_fragment(segment: &[OwnedLexToken]) -> Option<Vec<KeywordAction>> {
    parse_ability_line(segment).or_else(|| {
        let words = crate::cards::builders::compiler::token_word_refs(segment);
        (matches!(
            words.as_slice(),
            ["can't", "be", "blocked"]
                | ["cant", "be", "blocked"]
...
fn wrap_conditioned_animation_static_ability(
    ...
    if let Some(condition) = condition
        && let Some(conditioned) = ability.with_condition(condition.clone())
    {
        return conditioned.into();
    }
...
pub(crate) fn parse_anthem_with_trailing_segments_line(
    ...
    let direct_have_tail = if anthem_word_slice_starts_with(
        &crate::cards::builders::compiler::token_word_refs(&tail_tokens),
        &["and", "have"],
    ) {
    ...
    for raw_segment in split_anthem_trailing_segments_preserving_granted_abilities(&tail_tokens) {
        ...
        if anthem_word_slice_starts_with(
            &segment_words,
            ...
            .is_some_and(|word| *word == "has" || *word == "have")
        {
            ...
            if let Some(triggered) = parse_triggered_granted_ability(&ability_tokens)? {
                let display = format!(
                    "{} has {}",
                    clause_words.join(" "),
                    crate::cards::builders::compiler::token_word_refs(&ability_tokens).join(" ")
                );
...
pub(crate) fn parse_conditional_all_creatures_able_to_block_line(
    ...
    let all_words = all_words_storage
        ...
        .map(String::as_str)
        .collect::<Vec<_>>();
    if !anthem_word_slice_starts_with(&all_words, &["as", "long", "as"]) {
        return Ok(None);
    }
...
pub(crate) fn parse_source_can_attack_as_though_no_defender_as_long_as_line(
    ...
    let normalized = crate::cards::builders::compiler::token_word_refs(tokens)
        ...
        .collect::<Vec<_>>();
    let Some(can_idx) = anthem_find_word_sequence_index(
        &normalized,
        &[
...
pub(crate) fn parse_as_long_as_condition_can_attack_as_though_no_defender_line(
    ...
    let normalized = crate::cards::builders::compiler::token_word_refs(tokens)
        ...
        .map(|word| if word == "didn't" { "didnt" } else { word })
        .collect::<Vec<_>>();
    if !anthem_word_slice_starts_with(&normalized, &["as", "long", "as"]) {
        return Ok(None);
    }
...
pub(crate) fn parse_gets_and_attacks_each_combat_if_able_line(
    ...
    let attack_tail = crate::cards::builders::compiler::token_word_refs(&tokens[attack_idx..]);
...
pub(crate) fn parse_has_base_power_toughness_static_line(
    ...
    if starts_with_until_end_of_turn(&subject_words)
        || anthem_word_slice_starts_with(&subject_words, &["until", "your", "next", "turn"])
    {
        return Ok(None);
    }
...
pub(crate) fn parse_isnt_creature_line(
    ...
    let Some(verb_word_idx) = anthem_find_word_index(&clause_words, |word| {
